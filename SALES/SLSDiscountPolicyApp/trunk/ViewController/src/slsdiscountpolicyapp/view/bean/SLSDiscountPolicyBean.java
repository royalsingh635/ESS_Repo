...
public class SLSDiscountPolicyBean {
    ...
    private Timestamp currDt = null;

    public void setCurrDt(Timestamp currDt) {
        this.currDt = currDt;
    }
    ...
    public Timestamp getCurrDt() {
        if(currDt == null){
            ...
            try {
                currDt = new Timestamp(currDt.dateValue() + " 00:00:00");
    ...
    public void expDtVALID(FacesContext facesContext, UIComponent uIComponent, Object object) {
        if(object != null && startDtBinding.getValue() != null){
            ...
            if(strtDate.compareTo(expDate) > 0){
                throw new ValidatorException(new FacesMessage(ADFModelUtils.resolvRsrc("MSG.2283"),ADFModelUtils.resolvRsrc("MSG.2284"))); //Invalid Expiry Date !
    ...
    public void saveACTION(ActionEvent actionEvent) {
        ...
        if(binding.getErrors().isEmpty()){
            ...
            StringBuilder saveMsg =
                new StringBuilder("<html><body><b><p style='color:red'>" + ADFModelUtils.resolvRsrc("MSG.1260")//"Saved successfully !" 
                                                                        
                                 + "</p></b>");
            saveMsg.append(ADFModelUtils.resolvRsrc("MSG.2290"));        //Discount entries have been saved successfully.
            ...
            StringBuilder saveMsg =
                new StringBuilder("<html><body><b><p style='color:red'>" + ADFModelUtils.resolvRsrc("MSG.2291") +              //Error in saving data !
                                  "</p></b>");
            saveMsg.append(ADFModelUtils.resolvRsrc("MSG.2292"));      //There have been an error while saving data. Please try again or contact ESS.
            saveMsg.append("</body></html>");
            FacesMessage msg = new FacesMessage(saveMsg.toString());
    ...
    public Boolean ValidateAmount(){
        ...
        if(discVal.compareTo(new Number(0)) < 0){
            b = false;
            FacesMessage msg = new FacesMessage(ADFModelUtils.resolvRsrc("MSG.2285"),ADFModelUtils.resolvRsrc("MSG.2286")); //Invalid Discount Value !
            ...
            if(discVal.compareTo(new Number(100)) >= 0){
                b = false;
                FacesMessage msg = new FacesMessage(ADFModelUtils.resolvRsrc("MSG.1813"),ADFModelUtils.resolvRsrc("MSG.1813"));           //Invalid Percentage Value !
                                                                                                                                                    //Discount value should be greater between 0 to 99.99.
                msg.setSeverity(FacesMessage.SEVERITY_ERROR);
                FacesContext.getCurrentInstance().addMessage(discValBinding.getClientId(), msg);
            }  
        }else if(!isPrecisionValid(26, 6, discVal)){
            b = false;
            FacesMessage msg = new FacesMessage(ADFModelUtils.resolvRsrc("MSG.2288"),ADFModelUtils.resolvRsrc("MSG.2289"));               //Invalid Precision Value !
                                                                                                                                                    //Discount value can have maximum Precision of (26,6).
            msg.setSeverity(FacesMessage.SEVERITY_ERROR);
            FacesContext.getCurrentInstance().addMessage(discValBinding.getClientId(), msg);
        }
    ...
    public void effectiveDtVCL(ValueChangeEvent valueChangeEvent) {
        valueChangeEvent.getComponent().processUpdates(FacesContext.getCurrentInstance());
        this.getBindings().getOperationBinding("searchForEdit").execute();
        AdfFacesContext.getCurrentInstance().addPartialTarget(discountTableBinding);
        AdfFacesContext.getCurrentInstance().addPartialTarget(switcherBinding);
