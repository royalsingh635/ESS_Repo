...
public class SlSSecondarySaleAMImpl extends ApplicationModuleImpl implements SlSSecondarySaleAM {
        ...
        if (currentRow != null && currentRow.getAttribute("SsId") == null) {
            try {
                ...
                FacesMessage message =
                    new FacesMessage(ADFModelUtils.resolvRsrc("MSG.1782")); //There have been an error in calling function 'GEN_DISP_DOC_ID' !
    ...
    /*
     * Method to check the Item Qty for the shipment Item.
     */
    public Boolean chkForShipmentQty(){
        Boolean flag=true;
     
        String shipmntId=null;
        String ssId=null;
        String cld_id = resolvEl("#{pageFlowScope.GLBL_APP_CLD_ID}").toString();
        Integer sloc_id = Integer.parseInt(resolvEl("#{pageFlowScope.GLBL_APP_SERV_LOC}").toString());
        String org_id = resolvEl("#{pageFlowScope.GLBL_APP_USR_ORG}").toString();
        ...
        if(this.getSlsSs1().getCurrentRow()!=null){
            shipmntId=this.getSlsSs1().getCurrentRow().getAttribute("ShipId").toString();
            ssId=this.getSlsSs1().getCurrentRow().getAttribute("DocId").toString();
            System.out.println("Ship Id is "+shipmntId+"------"+"SecondarySales Id---"+ssId);
        }
        ViewObjectImpl chk1 = this.getShpmntQtyChk1();
      
        chk1.setNamedWhereClauseParam("SLOCIDBIND", sloc_id);
        chk1.setNamedWhereClauseParam("CLDIDBIND", cld_id);
        chk1.setNamedWhereClauseParam("ORGIDBIND", org_id);
        chk1.setNamedWhereClauseParam("HOORGIDBIND", ho_org_id);
        chk1.setNamedWhereClauseParam("DOCIDBIND", shipmntId);
        chk1.setNamedWhereClauseParam("SsDocIdBind", ssId); 
        chk1.executeQuery();
        System.out.println("Row Count"+chk1.getRowCount());
        RowSetIterator itr = chk1.createRowSetIterator(null);
        int i=0;
        while(itr.hasNext()){
           
            Number itmQty=new Number(0);
            Row next = itr.next();
            System.out.println("Next row is      "+next);
            System.out.println(next.getAttribute("BalQty"));
            Number itmQtyChk = (Number)next.getAttribute("BalQty");
            Row[] ftr = this.getSlsSsItm1().getFilteredRows("ItmId", next.getAttribute("ItmId"));
        System.out.println("The iterator runs ----->"+i+++"Times");
            if(ftr.length>0){
                for(Row f:ftr){
                   Number itmQtyItr =(Number) f.getAttribute("ItmQty");
                itmQty=itmQtyItr.add(itmQty);
                }
                System.out.println("Iterator Itm Qty is "+itmQty);
                System.out.println("Balance Qty "+itmQtyChk);
                if(itmQtyChk.compareTo(itmQty)<0){
                    flag= false;
                    System.out.println("The iterator runs in less than flag ----->"+i+"Times");
                    return flag;
                }
                else if(itmQtyChk.compareTo(itmQty)>0||itmQtyChk.compareTo(itmQty)==0){
                    flag= true;
                    System.out.println("The iterator runs in greater than flag ----->"+i+"Times");
    ...
    /**
     * Container's getter for ShpmntQtyChk1.
     * @return ShpmntQtyChk1
     */
    public ViewObjectImpl getShpmntQtyChk1() {
        return (ViewObjectImpl) findViewObject("ShpmntQtyChk1");
    }
}
