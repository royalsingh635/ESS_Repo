...
public class EbizSlsMobileWSAMImpl extends ApplicationModuleImpl {
    ...
    public Map getCatgDisBaseList() {
        ...
        Map<String, List> category = new HashMap<String, List>();
    ...
    public Map getDiscPolicyList(String CldId, String OrgId, String HoOrgId, Integer SlocId, String ItmNmStr,
        ...
        Map<String, List> discPolicy = new HashMap<String, List>();
        HashMap<Object, Object> h;
        List list = new ArrayList();
        ...
        while (ct.hasNext()) {
            ...
            h.put("EffectiveDt", next.getAttribute("EffectiveDt"));
        ...
        case 21502:
            {
                while (ct.hasNext()) {
                    ...
                    h.put("crtDt", next.getAttribute("DocDt"));
    ...
    public Map getSalesOrderDetail(Integer SlocId, String CldId, String OrgId, String HoOrgId, String docId,String itmId){
        ...
        while(ctr.hasNext()){
            ...
            h.put("DocDt", row.getAttribute("DocDt"));
            ...
            h.put("TotAmtSp", row.getAttribute("TotAmtSp"));
        ...
        while(ctr1.hasNext()){
            if (itmId == null) {
                ...
                h.put("ItmDiscTyp", row.getAttribute("ItmDiscTyp"));
                h.put("DiscVal", row.getAttribute("DiscVal"));
                h.put("ItmAmtSp", row.getAttribute("ItmAmtSp"));
                a.add(h);
            }else{
                Row row = ctr1.next();
     ...
     * @return
     */
    public Map updateSalesOrderData(Integer SlocId, String CldId, String OrgId, String HoOrgId, String docId,HashMap modifiedData){
        EbizSlsMobileWSAMImpl am = getAm();
        Map v = new HashMap();
        if(modifiedData != null){
            Set entrySet = modifiedData.entrySet();
            Iterator itr = entrySet.iterator();
            ArrayList<HashMap> soItmDtlList = (ArrayList<HashMap>)modifiedData.get("SoDetail");
            ArrayList<HashMap> soDtlList = (ArrayList<HashMap>)modifiedData.get("SoItmDetail");
            Boolean itmUpdSucessFul = false;
            String itmId = null;
            for(HashMap m : soItmDtlList){
                Object ItmIdO = m.get("ItmId");
                Object ItmQtyO = m.get("ItmQty");
                Object DiscValO = m.get("DiscVal");
                itmId = (ItmIdO == null ? "" : ItmIdO.toString());
                Number ItmQty = (ItmQtyO == null ? StaticValue.NUMBER_ZERO : (Number)ItmQtyO);
                Number DiscVal = (DiscValO == null ? StaticValue.NUMBER_ZERO : (Number)DiscValO);
                if(!("".equals(itmId) || ItmQty.compareTo(StaticValue.NUMBER_ZERO) <=0 || ItmQty.compareTo(StaticValue.NUMBER_ZERO) <=0)){
                    Object callFunction =
                        ADFModelUtils.callFunction(am, new StringBuilder("SLS.FN_UPDT_SO_ITM_DTLS_WS(?,?,?,?,?,?,?,?)"), new Object[] {
                                                   SlocId, CldId, OrgId, HoOrgId, docId, itmId, ItmQty, DiscVal
                    }, Types.INTEGER);
            ...
            if(itmUpdSucessFul){
                v = getSalesOrderDetail(SlocId, CldId, OrgId, HoOrgId, docId, itmId);
            }else{
                v = new HashMap();
                v.put("SoDetail", -1);
                v.put("SoItmDetail", -1);
