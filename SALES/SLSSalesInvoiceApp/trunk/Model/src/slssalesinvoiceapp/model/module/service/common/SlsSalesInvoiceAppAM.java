...
public interface SlsSalesInvoiceAppAM extends ApplicationModule {
   ...
   // boolean getSlsInvdtlsCount();
    ...
    Integer addShipmentToInv();

    Integer validateShipmentBeforeAdd();

    void refreshTax();

    Boolean checkItemValueChanged();

    Integer addSoToInvFrSrvcInv();

    StringBuilder getExportLink();

    Boolean retValForCustNm();

    Boolean isPymntRecvedAgnstInv();

    void callPostChange();

    void resetLOVonCustomerVCE();

    Boolean chkCcApplicableOrNot();

    void generateCostCenterHeaderWise();

    void iterateSlsInvShpItm();

    void deleteCostCenterForItem();

    void sendDateFromTempCcToSlsInvCc();

    Integer chkCostCenterEnable_Disable();
    ...
    void checkTrforItem(Integer UserId, String itmId);

    void filterAutoOtherChargesVo();

    Integer validateHeaderBeforeAdd();

    void resetTaxableAmount();

    void filterShipForIntm();

    void putIntimation(String intmDocId, String intmDispId);

    Boolean isAdvanIntmSearchApplicable();

    Boolean chkprofitCenterValidData();

    Boolean chkCurConvSource();

    Boolean validateNewItemRate(BigDecimal newRate);

    Boolean insertTrLinesForMismatch(String docId);

    Boolean isInvoiceValueValid();
}
