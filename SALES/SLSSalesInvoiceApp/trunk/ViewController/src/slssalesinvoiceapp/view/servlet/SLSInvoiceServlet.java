...
public class SLSInvoiceServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            StringBuilder query =
                new StringBuilder("SELECT NVL(A.EXCISE_TRD_ORG,'N')\n" + "            FROM APP.ORG$SLS$PRF A WHERE " +
                                  "  A.SLOC_ID = " + Integer.parseInt(request.getParameter("slocid")) +
                                  "  AND A.CLD_ID = '" + request.getParameter("cldid").toString() +
                                  "'  AND A.ORG_ID = '" + request.getParameter("Org").toString() +
                                  "' AND A.HO_ORG_ID = '" + request.getParameter("hoOrgid").toString() + "'");
            ...
            rs2 = st.executeQuery(query.toString());
            String exciseTradeOrg = "N";
            while (rs2.next()) {
                exciseTradeOrg = rs2.getString(1);
            }
            System.out.println(exciseTradeOrg + " Value result after query");
            ...
            path = path + "SLS/";
            //path = "/home/oracle/DeployMent/Report/SLS/";
            if (linkType.equalsIgnoreCase("invdetail")) {
                System.out.println(request.getParameter("invType") + " Invoice Type +++++++++++++++++++++");
                /*   Set Report Name for Service Invoice */
                if (Integer.parseInt(request.getParameter("invType")) == 795) {
                    reportName = "SLS_Inv_DetailReport_Service";
                    System.out.println(reportName);
                /*   Set Report Name for Trading Invoice */
                } else if(Integer.parseInt(request.getParameter("invType")) == 883) {
                    reportName = "Sls_Inv_DetailReport_Trading";
                    System.out.println(reportName);
                }
                else {
                    if (exciseTradeOrg.equalsIgnoreCase("Y")) {
                        reportName = "SLS_Inv_DetailReport_Excise";
                        System.out.println(reportName);
                    } else {
                        reportName = "SLS_Inv_DetailReport";
                        System.out.println(reportName);
                    }
                }
            }
            ...
            if (request.getParameter("invType").equals("")) {
                ...
                invType = Integer.parseInt(request.getParameter("invType"));
            }
            ...
            if ("pdf".equalsIgnoreCase(reporttype)) {
                ...
                response.setHeader("Content-Disposition", "attachement; filename=\"Sales Invoice.pdf\"");
