        ...
        if (row.getAttribute("CurrIdSp") != null) {
            ...
            -1-Initialized */
                    ...
                    if (state.equals(new Integer(2))) {
                        Integer ret =
                            (Integer)(callStoredFunction(Types.INTEGER, "MM.MM_INS_EO_ITM_HIST(?,?,?,?,?)", new Object[] { rowimpl.getCldId(),
                                                                                                                           rowimpl.getSlocId(),
                                                                                                                           rowimpl.getOrgId(),
                                                                                                                           rowimpl.getEoId(),
                                                                                                                           rowimpl.getItmId() }));
                ...
                FacesContext fc = FacesContext.getCurrentInstance();
                FacesMessage msg = new FacesMessage("Currency not valid or Conversion not define for this Currency.");
                msg.setSeverity(FacesMessage.SEVERITY_ERROR);
                fc.addMessage(suppCurrbinding.getClientId(), msg);
            }
        } else {
            FacesContext fc = FacesContext.getCurrentInstance();
            FacesMessage msg = new FacesMessage("Please select Supplier's Currency");
            msg.setSeverity(FacesMessage.SEVERITY_ERROR);
            fc.addMessage(suppCurrbinding.getClientId(), msg);
     ...
     /*        RowQualifier rq = new RowQualifier(am.getItmIdLOV1());
            ...
            if (itmId != null) {
               ...
               */  /*  for(Row row:r)
                ...
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Select valid Item.",
                                                              null));
            ...
            if (uompur != null && uombs != null) {
                ...
                BigDecimal ret =
                    (BigDecimal)(callStoredFunction(Types.NUMERIC, "APP.FN_GET_UOM_CONV_FCTR(?,?,?,?,?,?)",
                                                    new Object[] { sloc_id, cld_id, org_id, itmId, uompur, uombs }));
