...
public class Rqmtbean {
    ...
    public void editActionListen(ActionEvent actionEvent) {
        ...
        if(flag.equalsIgnoreCase("Y")){
           ...
           // setMode("Y");
            ...
            FacesMessage message = new FacesMessage("Requirement Area Can't Editable because it is Already in used");   
         ...
         if(OBinding.getErrors().isEmpty()){
           ...
           operationBinding.execute();
              if(operationBinding.getErrors().isEmpty()){
                  ...
                  FacesMessage message = new FacesMessage("Record Not Save");   
                  message.setSeverity(FacesMessage.SEVERITY_ERROR);   
                  FacesContext fc = FacesContext.getCurrentInstance();   
                  fc.addMessage(null, message); 
              }
    ...
    public void deleteActionListen(ActionEvent actionEvent) {
        ...
        if(flag.equalsIgnoreCase("Y")){
            ...
            FacesMessage message = new FacesMessage("Requirement Area Can't Deletable because it is Already in used");   
            message.setSeverity(FacesMessage.SEVERITY_ERROR);   
            FacesContext fc = FacesContext.getCurrentInstance();   
            fc.addMessage(null, message);  
        }
    ...
    public void pop1dialogListner(DialogEvent dialogEvent) {
        if(dialogEvent.getOutcome().name().equalsIgnoreCase("yes")){
                  ...
                  if(binding.getErrors().isEmpty()){
                     ...
                     // binding1.execute();
                      String message1=resolvElDCMsg("Record Delete Successfully").toString();
                      FacesMessage message = new FacesMessage(message1);   
                          message.setSeverity(FacesMessage.SEVERITY_INFO);   
                          FacesContext fc = FacesContext.getCurrentInstance();   
                          fc.addMessage(null, message);  
                  }else{
                      String message1=resolvElDCMsg("Record Not Deleted").toString();
                      FacesMessage message = new FacesMessage(message1);   
                          message.setSeverity(FacesMessage.SEVERITY_ERROR);   
                          FacesContext fc = FacesContext.getCurrentInstance();   
                          fc.addMessage(null, message);
