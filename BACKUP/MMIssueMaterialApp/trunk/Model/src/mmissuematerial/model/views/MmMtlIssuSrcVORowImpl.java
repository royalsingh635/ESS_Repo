...
public class MmMtlIssuSrcVORowImpl extends ViewRowImpl {
    ...
    public String getDocIdSrcDisp() {
        ...
        adflog.info("Inside gettter");
        adflog.info("docSrcId  --"+docSrcId+ " getIssuDocType  "+getIssuDocType());
        if(docSrcId!=null && getIssuDocType()!=null){
            ...
            if(getIssuDocType()==326){
                Row[] rrMrs = am.getLovMRSDocNo().getFilteredRows("MrsTxnId", docSrcId);
                adflog.info("lenth  do mrs "+rrMrs.length);
            if(rrMrs.length>0){
                docSrcIdDisp = rrMrs[0].getAttribute("MrsNo").toString();
            }
            adflog.info("doc  do "+docSrcId);
            adflog.info("display do "+docSrcIdDisp);
            }else if(getIssuDocType()==330 || getIssuDocType()==366){
                Row[] rrTr =am.getTransferOrderDocNo().getFilteredRows("DocId", docSrcId);
                adflog.info("lenth  do  tr "+rrTr.length);
                if(rrTr.length>0){
                    docSrcIdDisp = rrTr[0].getAttribute("TrfNo").toString();
                }
                adflog.info("doc  do tr "+docSrcId);
                adflog.info("display do tr "+docSrcIdDisp);
            }else if(getIssuDocType()==329){
                Row[] rrSo =am.getLovSlsSoNo().getFilteredRows("DocId", docSrcId);
                adflog.info("lenth  do  tr "+rrSo.length);
                if(rrSo.length>0){
                    docSrcIdDisp = rrSo[0].getAttribute("SoId").toString();
                }
                adflog.info("doc  do So "+docSrcId);
                adflog.info("display do So "+docSrcIdDisp);
            }else if(getIssuDocType()==432){
                Row[] rrShip =am.getLovSlsShipmntNo().getFilteredRows("DocId", docSrcId);
                adflog.info("lenth  do  shipmnt "+rrShip.length);
                if(rrShip.length>0){
                    docSrcIdDisp = rrShip[0].getAttribute("ShipmntId").toString();
                }
                adflog.info("doc  do shipment "+docSrcId);
                adflog.info("display do shipment "+docSrcIdDisp);
            }else if(getIssuDocType()==543){
                    Row[] rrKit =am.getLovKitProd().getFilteredRows("DocId", docSrcId);
                    adflog.info("lenth  do  tr "+rrKit.length);
                    if(rrKit.length>0){
