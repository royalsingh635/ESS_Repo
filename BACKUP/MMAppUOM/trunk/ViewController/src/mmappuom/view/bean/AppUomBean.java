...
import mmappuom.model.views.AppUomConvStdVORowImpl;
...
public class AppUomBean {
   ...
   static String mode = "A";
        ...
        if (dialogEvent.getOutcome().name().equals("ok")) {
          ...
          adfLog.info("MOde  :::::"+getMode());
            ...
            String table = "APP$UOM$CONV$STD";
             uomId = callStoredFunction(VARCHAR, "APP.FN_APP_GEN_ID(?,?,?,?,?,?)", new Object[] {slocid, cldid,hoOrgId, null, table,null}).toString();
            System.out.println("user id setted value is "+uomId);
                ...
                System.out.println("usrId created is "+currentRow.getAttribute("UsrIdCreateDt")+" try value is "+(Date)Date.getCurrentDate());
                BindingContainer bindings = getBindings();
           ...
           // String cldid =(String)resolveEl("#{pageFlowScope.GLBL_APP_CLD_ID}");
            ...
            String del = callStoredFunction(VARCHAR, "PKG_UOM.IS_UOM_DELETABLE(?,?,?,?)", new Object[] {slocid ,cldid ,null , uomid}).toString();
    ...
    public void baseUomValueChange(ValueChangeEvent valueChangeEvent) {
        ...
        if (valueChangeEvent.getNewValue().toString().equalsIgnoreCase("true")) {
           ...
           Integer ret =
                (Integer)callStoredFunction(Types.INTEGER, "PKG_UOM.UPDT_CONV_FCTR(?,?,?)", new Object[] {slocid,cldid,currRow.getAttribute("UomId")});  
    ...
    public void deleteUomButton(ActionEvent actionEvent) {
        ...
        String uom = row.getAttribute("UomId").toString();
         ...
         String del = callStoredFunction(VARCHAR, "PKG_UOM.IS_UOM_DELETABLE(?,?,?,?)", new Object[] {slocid,cldid,ho_org_id, uom}).toString();
   ...
   //  if(curr.getAttribute("UomIdSrc")!=null && curr.getAttribute("UomIdDest")!=null) {
         ...
         if(object!=null)
         {
             ...
             {
                     throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,"Conversion Factor must be Greater than Zero.",null));      
         
                 }
             }
         else
         {
                 throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,"Conversion Factor Required.",null));      
             }
     }
