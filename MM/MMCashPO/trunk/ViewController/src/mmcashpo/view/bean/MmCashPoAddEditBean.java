...
public class MmCashPoAddEditBean implements Serializable {
    ...
    public void saveButton(ActionEvent actionEvent) {
         ...
         * */
           ...
           Integer count123 =new Integer(0);
            while (rsi.hasNext()) {
                Row rw = rsi.next();
            /**code need to be changed because of itm id crash in case of duplicate items */
                adfLog.info("item id and uom is "+rw.getAttribute("ItmId")+"  "+rw.getAttribute("ItmUom"));
                if(rw.getAttribute("ItmId")!=null &&  rw.getAttribute("ItmUom")!=null){
            ...
            adfLog.info("total number of items in this document "+count123);
            if(count123.compareTo(new Integer(0))==1){
                String msg2 =resolvEl("Item or Unit null or same item with same unit exists").toString();
            ...
            if ("Y".equalsIgnoreCase(appTax)) {
                ...
                FacesMessage message =
                    new FacesMessage(resolvEl("#{bundle['MSG.292']}").toString() +
                                     itmNameConcat.substring(0, itmNameConcat.length() - 1) +
                                     resolvEl("#{bundle['MSG.293']}").toString());
                ...
                if (currRow.getAttribute("CpoId") == null) {
                    ...
                    String cpoId = (String) (callStoredFunction(Types.VARCHAR, "MM.FN_MM_GEN_ID (?,?,?,?)", new Object[] {
                                                                SlocId, cldId, p_org_id, "MM$CPO"
                    }));
                    ...
                    if (commitCount == 0) {
                        ...
                        if (WfNum != null) {
                            ...
                            if (level > -1) {
                                ...
                                StringBuilder msg =
                                    new StringBuilder("<html><body><p>" + resolvEl("#{bundle['MSG.298']}").toString() +
                                                      "</p>");
                                msg.append("<p>" + resolvEl("#{bundle['MSG.299']}").toString() + "<b>" + amtSp +
                                           "</b></p>");
                                msg.append("<p>" + resolvEl("#{bundle['MSG.300']}").toString() + "<b>" + amtBs +
                                           "</b></p>");
                            ...
                            String msg2 =
                                "Workflow not created for Cash Purchase.Please add new Workflow to Cash Purchase.";
                            FacesMessage message2 = new FacesMessage(msg2);
                            message2.setSeverity(FacesMessage.SEVERITY_ERROR);
                            FacesContext fc = FacesContext.getCurrentInstance();
                            fc.addMessage(null, message2);
    ...
    public void deleteSelectedItem(ActionEvent actionEvent) {
        ...
        while (rit.hasNext()) {
            Row row = rit.next();
            if (row.getAttribute("MarkedForDelete") != null &&
                row.getAttribute("MarkedForDelete").toString().equalsIgnoreCase("Y")) {
                if (row.getAttribute("ItmId") != null) {
                    rqTax.setWhereClause("DocId= '" + row.getAttribute("DocId").toString() + "' and ItmId= '" +
                    ...
                    if (rTax.length > 0) {
                        ...
                        rqTaxLine.setWhereClause("DocId= '" + row.getAttribute("DocId").toString() + "' and ItmId= '" +
                                                 row.getAttribute("ItmId").toString() + "' and TaxRuleId= " +
                                                 rTax[0].getAttribute("TaxRuleId") + "");
   ...
   OperationBinding operationBinding = getBindings().getOperationBinding("Createwithparameters2");
    ...
    public void taxPopupCancelList(PopupCanceledEvent popupCanceledEvent) throws SQLException {
        ...
        if (taxid1 != null) {
            ...
            BigDecimal ret1 =
                (BigDecimal) callStoredFunction(NUMBER, "MM.MM_INS_CPO_TR_LINES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", new Object[] {
    ...
    public void discountValueValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
       ...
       Number amt = (Number)itmQty.mul(itmPrice); */
            ...
            if (itmDicsTypeBind.getValue().equals("A")) {
                if (((Number) orderQty.getValue()).compareTo(zero) == 1 &&
                    ((Number) itmPrice.getValue()).compareTo(zero) == 1) {
                    if (discValue.compareTo(amt) > 0) {
                        throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                                      resolvEl("#{bundle['MSG.304']}").toString() + amt,
                                                                      null));
                    }
                }
            }
    ...
    public void orderQtyValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (p_quot_item_id != null && orderQtyVal != null) {
            ...
            Integer fy = (Integer) (callStoredFunction(Types.INTEGER, "APP.GET_ORG_FY_ID (?,?,?)", new Object[] {
                                                       cldId, p_org_id, dt
            }));
            flag = (String) callStoredFunction(Types.VARCHAR, "MM.PKG_MM_STK.IS_ORD_QTY_VALID(?,?,?,?,?,?,?)", new Object[] {
                                               p_sloc_id, cldId, P_ho_org_id, p_org_id, p_quot_item_id, fy, orderQtyVal
            });
        }
        if (flag.equalsIgnoreCase("N")) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvEl("#{bundle['MSG.306']}").toString(), null));
        }

        if (orderQtyVal.compareTo(zero) <= 0) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvEl("#{bundle['MSG.307']}").toString(), null));

        }
        if (totFlag.equals(false)) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvEl("#{bundle['MSG.95']}").toString(), null));
        }
    }
    ...
    public void itmPriceValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (itmPriceVal.compareTo(zero) <= 0) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvEl("#{bundle['MSG.309']}").toString(), null));
        }
        if (totFlag.equals(false)) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvEl("#{bundle['MSG.95']}").toString(), null));
    ...
    public void historyIcon(ActionEvent actionEvent) {
        ...
        if (itmRow.getAttribute("ItmId") != null) {
            ...
            callStoredFunctionHistory("MM.PKG_MM_STK.GET_STK(?,?,?,?,?,?,?,?,?,?)", new Object[] {
                                      SlocId, cldId, itmId, fy, p_org_id, date, null
            });
    ...
    public void tranItmNameValueChange(ValueChangeEvent valueChangeEvent) {
        MmCashPOAMImpl am = (MmCashPOAMImpl) resolvElDC("MmCashPOAMDataControl");
        ViewObjectImpl e = am.getLovItmIdForadd1();
        Row curr = am.getMmCpo1().getCurrentRow();

        RowQualifier rq = new RowQualifier(e);
        rq.setWhereClause("CldId='" + cldId + "' and SlocId=" + slocId + " and OrgId ='" + orgId + "' and HoOrgId='" +
                          hoOrgId + "' and ItmDesc='" + valueChangeEvent.getNewValue().toString() + "'");
        Row[] rws = e.getFilteredRows(rq);
        if (rws.length > 0) {
    ...
    public void discValonPopupValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (disctypeCpoBind.getValue().equals("A")) {
            if (discval.compareTo(transSpAmt) > 0) {
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                              resolvEl("#{bundle['MSG.262']}").toString() + transSpAmt,
                                                              null));
            }
        }
    ...
    public void ocSpValuevalidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (transTypeBind.getValue().equals(1) && orderQtyVal.compareTo(totalCpoSpAmt) >= 0) {
            ...
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvEl("#{bundle['MSG.302']}").toString(), null));
        }
        if (orderQtyVal.compareTo(zero) <= 0) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                ...
                while (rsitm.hasNext()) {
                    ...
                    if (rw.getAttribute("DocId").toString().equalsIgnoreCase(docId)) {
                        ...
                        if (ex.equals("Y")) {
                            ...
                            //TaxableAmt
                            adfLog.info("tax after discount flag value is " +
                                        povo.getCurrentRow().getAttribute("TaxAfterDiscFlg"));
                            ...
                            adfLog.info("tax rule flag is " + taxruleflg + "taxable amount is " +
                                        newrow.getAttribute("TaxableAmt") + "taxble set amt " +
                                        rw.getAttribute("TranItmAmtSp") + " price and qty " +
                                        rw.getAttribute("TranPriceQty"));
                            //create trlines
                            Number taxAmt = new Number(0);
                            BigDecimal ret =
                                (BigDecimal) callStoredFunction(NUMBER,
                                                                "MM.MM_INS_CPO_TR_LINES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", new Object[] {
    ...
    public void defaultTaxAction(ActionEvent actionEvent) {
        ...
        if ("Y".equalsIgnoreCase(exmtdflag)) {
            ...
            FacesContext fc = FacesContext.getCurrentInstance();
            fc.addMessage(null, message2);
        } else if ("N".equalsIgnoreCase(exmtdflag)) {
            OperationBinding applyTax = getBindings().getOperationBinding("ApplyTax");
            applyTax.execute();
        }
    }
