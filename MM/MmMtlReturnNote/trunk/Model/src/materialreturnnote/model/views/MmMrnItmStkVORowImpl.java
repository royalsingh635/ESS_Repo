...
public class MmMrnItmStkVORowImpl extends ViewRowImpl {
    ...
    public String getTransItmSerialChk() {
        ...
        if (itmId != null) {
            ...
            Row[] xx = itmVO.getFilteredRows("ItmId", itmId);
            if (xx.length > 0 && xx[0].getAttribute("SerializedFlg") != null) {
    ...
    public Number getTranstotIssLotQty() {
        ...
        if (getAttributeInternal(TRANSTOTISSLOTQTY) != null) {
            adf_log.info("lot id and item id is " + getLotId() + "item id is " + getItmId() + " curr rqmt id is " +
                         mrnVo.getAttribute("RqmtAreaId"));
            Number avlQty = null;
            if (getLotId() != null && getItmId() != null && mrnVo.getAttribute("RqmtAreaId") != null &&
                mrnVo.getAttribute("DocIdIssu") != null) {

                MmMrnItmStkEOImpl eO = getMmMrnItmStkEO();
                BigDecimal avlQty1 = (BigDecimal) callStoredFunction(Types.NUMERIC, "MM_MRN_AVL_QTY(?,?,?,?,?,?,?)", new Object[] {
                                                                     getCldId(), getSlocId(), getOrgId(), getLotId(),
                                                                     getItmId(), mrnVo.getAttribute("DocIdIssu"),
                                                                     mrnVo.getAttribute("RqmtAreaId")
                });
                try {
            ...
            return avlQty;
        } else if (getAttributeInternal(TRANSTOTISSLOTQTY) == null ||
                   ((Number) getAttributeInternal(TRANSTOTISSLOTQTY)).compareTo(new Number(0)) == 0) {
            adf_log.info("lot id and item id is " + getLotId() + "item id is " + getItmId() + " curr rqmt id is " +
                         mrnVo.getAttribute("RqmtAreaId"));
            Number avlQty = null;
            if (getLotId() != null && getItmId() != null && mrnVo.getAttribute("RqmtAreaId") != null) {
    ...
    public void setTransSrNo(String value) {
        /*   if(value != null ){
            System.out.println("-----------b inside settranssrno---------");
            setTotRetQty(new Number(1));
            setOkQty(new Number(1));
            setRejQty(new Number(0));
            setRwkQty(new Number(0));
            setScrpQty(new Number(0));
            System.out.println("-----------inside settranssrno---------");
        } */
