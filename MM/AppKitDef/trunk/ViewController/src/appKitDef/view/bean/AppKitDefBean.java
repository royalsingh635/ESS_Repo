...
public class AppKitDefBean {
        ...
        if ("N".equalsIgnoreCase(ret)) {
            ...
            FacesMessage msg =
                new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                 "Output Item used in another application . You can't add more Input Item .", null);
    ...
    public void edit(ActionEvent actionEvent) {
        ...
        if ("N".equalsIgnoreCase(ret)) {
            ...
            FacesMessage msg =
                new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                 "Output Item used in another application . You can't Edit it .", null);
            fc.addMessage(null, msg);
        } else if ("Y".equalsIgnoreCase(ret)) {
    ...
    public void inputItmVCL(ValueChangeEvent vCE) {
        ...
        if (count > 0) {
            FacesContext fc = FacesContext.getCurrentInstance();
            FacesMessage msg =
                new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.46']}").toString(), null);
    ...
    public void deleteInuptItm(ActionEvent actionEvent) {
        ...
        if ("N".equalsIgnoreCase(ret)) {
            ...
            FacesMessage msg =
                new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                 "Output Item used in another application . You can't Delete it .", null);
            fc.addMessage(null, msg);
        } else if ("Y".equalsIgnoreCase(ret)) {
            KitDefAppAMImpl am = (KitDefAppAMImpl) resolvElDC("KitDefAppAMDataControl");
    ...
    public void saveItm(ActionEvent actionEvent) {
        ...
        OperationBinding ob1 = (OperationBinding) bindings.getOperationBinding("chkInputItem");
        ob1.execute();
        if (ob1.getResult() != null) {
            Integer serFl = Integer.parseInt(ob1.getResult().toString());
            if (serFl.compareTo(new Integer(1)) == 0) {
                FacesContext fc = FacesContext.getCurrentInstance();
                FacesMessage msg =
                    new FacesMessage(FacesMessage.SEVERITY_WARN,
                                     "Non-Serialized output item donot contain Serialized input Item  ", null);
                fc.addMessage(null, msg);
                return;
            }
        }
        ...
        if (operationBinding.getErrors().isEmpty()) {
            FacesContext fc = FacesContext.getCurrentInstance();
            FacesMessage msg =
                new FacesMessage(FacesMessage.SEVERITY_INFO, resolvElDCMsg("#{bundle['MSG.91']}").toString(), null);
            fc.addMessage(null, msg);
        }
        this.form_readonly = true;
        this.output_itm = true;
    ...
    public void inputItmValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (inputItm.equalsIgnoreCase(ouputItmDesc)) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvElDCMsg("#{bundle['MSG.272']}").toString(), null));
        }
        /**------------For duplicate input item validation-----------------------*/
        int totalCount = v.getRowCount(); //get ALL rows
        int rangeSize = v.getRangeSize(); //all in range
    ...
    public void itmQtyValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (n == null) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvElDCMsg("#{bundle['MSG.273']}").toString(), null));
    ...
    public void thisWeekListener(ActionEvent actionEvent) {
        ...
        try {
            ...
            System.out.println(" date are ----"+daTime +"     "+ daTime1);
        } catch (ParseException pe) {
            pe.printStackTrace();
        }
        //itemSearch.setWhereClause("CREATE_DT between trunc('"+date+"') and trunc('"+currDate+"')");
        //itemSearch.setWhereClause("CREATE_DT >= to_date('" + daTime1 + "','yyyy/mm/dd') and CREATE_DT <= to_date('" +
          //                        daTime + "','yyyy/mm/dd')");
        
        itemSearch.setNamedWhereClauseParam("CldIdBind", cldId);
        itemSearch.setNamedWhereClauseParam("SlocIdBind", slocId);
        itemSearch.setNamedWhereClauseParam("OrgIdBind", orgId);
        itemSearch.setNamedWhereClauseParam("FromCreateDtBindVar", daTime1);
        itemSearch.setNamedWhereClauseParam("ToCreateDtBindVar", daTime);
        itemSearch.executeQuery();
        setDisable_link("W");
    }
    ...
    public void lastMonthListener(ActionEvent actionEvent) {
        ...
        try {
            ...
            daTime = new oracle.jbo.domain.Date(sqldate);
            daTime1 = new oracle.jbo.domain.Date(sqldate1);
            System.out.println(" month date are----"+daTime +"     "+ daTime1);
        } catch (ParseException pe) {
            pe.printStackTrace();
        }
        //itemSearch.setWhereClause("CREATE_DT between trunc('"+date+"') and trunc('"+currDate+"')");
    ...
    public void thisYearListener(ActionEvent actionEvent) {
        ...
        try {
            ...
            daTime = new oracle.jbo.domain.Date(sqldate);
            daTime1 = new oracle.jbo.domain.Date(sqldate1);
            System.out.println(" getting date inside year---"+ daTime + "  "+ daTime1);
        } catch (ParseException pe) {
            pe.printStackTrace();
        }
    ...
    public void outItmValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        if (object != null) {
            ...
            System.out.println(rArray.length + "  row lenth " + rq.getExprStr());
            Row cRow = v.getCurrentRow();
            int count = 0;
            String itemIdCur = "";
            for (Row r : rArray) {
                if (!r.equals(cRow)) {
                    System.out.println(r.getAttribute("OrgId") + " outItmValidator  :::" +
                                       r.getAttribute("OutputItmId"));
                    try {
    ...
    public void outItmVCL(ValueChangeEvent vce) {
        if (vce.getNewValue() != null) {
            ...
            rq.setWhereClause("CldId ='" + cld_id + "' and SlocId =" + sloc_id + " and OrgId ='" + org_id + "' ");
            Row[] rArray = v.getFilteredRows(rq);
            System.out.println(rArray.length + "  row lenth11 " + rq.getExprStr());
            Row cRow = v.getCurrentRow();
            int count = 0;
            String itemIdCur = "";
            for (Row r : rArray) {
                if (!r.equals(cRow)) {
                    System.out.println(" output vcl   :::");
                    try {
                        itemIdCur = r.getAttribute("OutputItmId").toString();
                    } catch (NullPointerException npe) {
                        System.out.println("NPE:" + npe);
            ...
            if (count > 0) {
                FacesContext fc = FacesContext.getCurrentInstance();
                FacesMessage msg =
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.91']}").toString(),
                                     null);
    ...
    public String noBtnAction() {
        // Add event code here...
        deleteOutputPopup.hide();
        return null;
    }

    public String yesBtnAction() {
        // Add event code here...
        BindingContainer bindings = getBindings();
        BindingContainer bindings1 = getBindings();
        OperationBinding operationBinding = bindings.getOperationBinding("Delete1");
        ...
        deleteItemPopUp.hide();
        return "back";
    }

    public String noPopupBtnAction() {
        // Add event code here...
        deleteItemPopUp.hide();
        return null;
    }
