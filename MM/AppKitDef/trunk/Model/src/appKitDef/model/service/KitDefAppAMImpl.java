...
public class KitDefAppAMImpl extends ApplicationModuleImpl implements KitDefAppAM {
     ...
     * @return AppKitNew1
     */
    public AppKitNewVOImpl getAppKitNew1() {
        return (AppKitNewVOImpl) findViewObject("AppKitNew1");
    }
    ...
    public String isItemDeletable() {
        ...
        if (appkit.getCurrentRow().getAttribute("OutputItmId") != null) {
            ...
            String Output1 = (callStoredFunctionNew(Types.VARCHAR, "mm.MM_IS_KIT_EDITABLE (?,?,?,?)", new Object[] {
                                                    cld_id, sloc_id, org_id, itmId
            })).toString();
    ...
    public Integer chkInputItem() {
        String cld_id = resolvEl("#{pageFlowScope.GLBL_APP_CLD_ID}").toString();
        Integer sloc_id = Integer.parseInt(resolvEl("#{pageFlowScope.GLBL_APP_SERV_LOC}").toString());
        String org_id = resolvEl("#{pageFlowScope.GLBL_HO_ORG_ID}").toString();
        ViewObjectImpl appkit = this.getAppKit1();
        if (appkit.getCurrentRow() != null) {
            Row cRowAppkit1 = appkit.getCurrentRow();
            if (cRowAppkit1.getAttribute("InputItmId") != null && cRowAppkit1.getAttribute("OutputItmId") != null) {
                System.out.println("out put " + cRowAppkit1.getAttribute("OutputItmId"));
                System.out.println("in put " + cRowAppkit1.getAttribute("InputItmId"));
                this.getItemView().setNamedWhereClauseParam("BindCldId", cld_id);
                this.getItemView().setNamedWhereClauseParam("BindSlocId", sloc_id);
                this.getItemView().setNamedWhereClauseParam("BindHoOrgId", org_id);
                this.getItemView().setNamedWhereClauseParam("BindItemId", cRowAppkit1.getAttribute("OutputItmId"));
                this.getItemView().executeQuery();
                Row rr[] = this.getItemView().getFilteredRows("ItmId", cRowAppkit1.getAttribute("OutputItmId"));
                if (rr.length > 0 && rr[0].getAttribute("SerializedFlg") != null) {
                    String serFlg = rr[0].getAttribute("SerializedFlg").toString();
                    if (serFlg.equalsIgnoreCase("N"))

                    {
                        this.getItemView().setNamedWhereClauseParam("BindCldId", cld_id);
                        this.getItemView().setNamedWhereClauseParam("BindSlocId", sloc_id);
                        this.getItemView().setNamedWhereClauseParam("BindHoOrgId", org_id);
                        this.getItemView().setNamedWhereClauseParam("BindItemId",
                                                                    cRowAppkit1.getAttribute("InputItmId"));
                        this.getItemView().executeQuery();
                        Row rr1[] = this.getItemView().getFilteredRows("ItmId", cRowAppkit1.getAttribute("InputItmId"));
                        if (rr1.length > 0 && rr1[0].getAttribute("SerializedFlg") != null) {
                            String serFlg1 = rr1[0].getAttribute("SerializedFlg").toString();
                            if (serFlg1.equalsIgnoreCase("Y")) {
                                return 1;
                            } else {
                                return 0;
                            }
     ...
     * @return ItemView1
     */
    public ViewObjectImpl getItemView() {
        return (ViewObjectImpl) findViewObject("ItemView");
    }
}
