...
import mminvoice.model.entities.MmInvcDtlEOImpl;
...
public class MmInvcDtlVORowImpl extends ViewRowImpl {
     ...
     */
    public MmInvcDtlEOImpl getMmInvcDtlEO() {
        return (MmInvcDtlEOImpl) getEntity(ENTITY_MMINVCDTLEO);
    }
    ...
    public String getTransDocNm() {
        ...
        adfLog.info("invoice type is in trans doc name" + type);

        Object obj = curr.getAttribute("InvcNo");
        String ret = "";
        if (type == 455 || type == 738 || type == 457) { //Purchase Invoice
            //Row[] rws=null;

            ViewObjectImpl rcptFilter = am.getLovRcptNoFltrDtl();
            rcptFilter.setNamedWhereClauseParam("bindCldId", getCldId());
            rcptFilter.setNamedWhereClauseParam("bindSlocId", getSlocId());
            rcptFilter.setNamedWhereClauseParam("bindOrgId", getOrgId());
            rcptFilter.executeQuery();
            ...
            /* if(obj==null)
                  ...
                  */
            Row[] rws = rcptFilter.getFilteredRows("DocId", this.getDocIdRcpt());
            if (rws.length > 0) {
        ...
        else if (type == 456) { //Transfer Invoice
            //  adfLog.info("invoice type is in inside supplimentary and tranfer "+type);
            Row[] rws = null;
            Object val = null;
            ...
            if (obj == null) {
                ...
                mtlIssuVo.executeQuery();
                //  adfLog.info("supplimentary ge query "+mtlIssuVo.getQuery());
            }
        ...
        /*    else if(type==457){ //Cash Purchase Invoice
            ...
            if (type == 455 || type == 492 || type == 737 || type == 739) {
                ret11 = (Object) (callStoredFunction(Types.VARCHAR, "MM.MM_GET_PO_NO(?,?,?,?)", new Object[] {
            ...
            if (rws.length > 0) {
                ret = rws[0].getAttribute("RcptNo").toString();
                adfLog.info("----supp inv rcpt no---->>>>"+ret);
                return ret;
            } else {
                return (String) getAttributeInternal(TRANSDOCNM);
            }
