...
public class BinprofileBean {
    ...
    private RichSelectOneChoice locIdBind;
    private RichInputText binditmqty;
    ...
    public void saveButton(ActionEvent actionEvent) {
        ...
        if (operationBinding.getErrors().isEmpty()) {
            ...
            setBtnMode("S");
            if (mode.equalsIgnoreCase("A")) {
                String msg=ADFModelUtils.resolvRsrc("MSG.89");
...
//                Bin Updated Successfully
                String msg=ADFModelUtils.resolvRsrc("MSG.479");
                FacesMessage editMsg = new FacesMessage(msg);
...
//                At start and end Space not allowed.
                String msg=ADFModelUtils.resolvRsrc("MSG.480");
...
//                Brackets not closed properly.
                String msg=ADFModelUtils.resolvRsrc("MSG.7");
...
//                Empty Brackets are not allowed.
                String msg=ADFModelUtils.resolvRsrc("MSG.170");
...
//                Invalid Bin name. Check content inside brackets.
                String msg=ADFModelUtils.resolvRsrc("MSG.481");
...
//            Invalid Bin name. Check content inside brackets.
            String error =ADFModelUtils.resolvRsrc("MSG.481");
//             resolvElDCMsg("#{bundle['MSG.481']}").toString();

            if (matcher.matches()) {
            } else {
//                Only Alphanumeric value and / - _ # :. ( ) and two consecutive special not allowed
                String s=ADFModelUtils.resolvRsrc("MSG.199");
    ...
    public void deleteBinLink(ActionEvent actionEvent) {
        ...
        String num = (String) (callStoredFunction(Types.VARCHAR, "APP.FN_IS_BIN_DELETABLE (?,?,?,?,?)", new Object[] {
                                                  CLDID, SLOCID, ORGID, whId, binId
        }));
        System.out.println("function output is " + num);
        if (num.equalsIgnoreCase("Y")) {
            showPopup(deleteCheckPopUp, true);
        } else {
//            This bin can not be deleted as it contains Items
            String s=ADFModelUtils.resolvRsrc("MSG.483");
...
//            Capacity must be a positive value
            String errMsg=ADFModelUtils.resolvRsrc("MSG.485");
            String s=ADFModelUtils.resolvRsrc("MSG.484");
//             resolvElDCMsg("#{bundle['MSG.485']}").toString();
    ...
    public void addItmDialogListener(DialogEvent dialogEvent) {
        ...
        if (dialogEvent.getOutcome().name().equals("ok")) {
          ...
          if(count >0){
            ...
            Number val=(Number)binditmqty.getValue();
            ...
            AdfFacesContext.getCurrentInstance().addPartialTarget(itmTableBind);
                ...
                String msg2=ADFModelUtils.resolvRsrc("MSG.2069");
//                 = resolvElDCMsg("#{bundle['MSG.2069']}").toString();
//                "Item Quantity must be greater zero."
                            FacesMessage message = new FacesMessage(msg2);
                               message.setSeverity(FacesMessage.SEVERITY_ERROR);
                               FacesContext fc = FacesContext.getCurrentInstance();
                               fc.addMessage(binditmqty.getClientId(), message);
    ...
    public void itmNameValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        System.out.println("SLOCID---" + SLOCID + "ORGID--" + ORGID + "CLDID--" + CLDID + "whId--" + whId + "binId--" +
                           binId + "inputItm--" + inputItm + "itmId--" + itmId);
        if (itmId != null) {
...
//             resolvElDCMsg("#{bundle['MSG.46']}").toString();
    ...
    public void itemValueChangeListener(ValueChangeEvent vce) {
        ...
        System.out.println("vce   SLOCID---" + SLOCID + "ORGID--" + ORGID + "CLDID--" + CLDID + "whId--" + whId +
                           "binId--" + binId + "inputItm--" + inputItm + "itmId--" + itmId);
        if (itmId != null) {
            ...
            rowQualifier.setWhereClause(" OrgId = '" + ORGID + "' and SlocId=" + SLOCID + " and CldId = '" + CLDID +
                                        "' and WhId = '" + whId + "' and BinId = '" + binId + "' and ItmId= '" + itmId +
                                        "'");
            //rowQualifier.setWhereClause(" OrgId = '" + ORGID + "' and SlocId=" + SLOCID + " and CldId = '"+CLDID+"' and WhId = '"+whId+"' and BinId = '"+binId+"'");
            Row[] rows = v.getFilteredRows(rowQualifier);
...
//                Duplicate Record Found.
    ...
    public void setLocIdBind(RichSelectOneChoice locIdBind) {
        this.locIdBind = locIdBind;
    }

    public RichSelectOneChoice getLocIdBind() {
        return locIdBind;
    }
    ...
    public void warehouseChangeListener(ValueChangeEvent valueChangeEvent) {
        ...
        getBindings().getOperationBinding("refreshLov").execute();
        AdfFacesContext.getCurrentInstance().addPartialTarget(locIdBind);
    }

    public void setBinditmqty(RichInputText binditmqty) {
        this.binditmqty = binditmqty;
    }

    public RichInputText getBinditmqty() {
        return binditmqty;
    }
}
