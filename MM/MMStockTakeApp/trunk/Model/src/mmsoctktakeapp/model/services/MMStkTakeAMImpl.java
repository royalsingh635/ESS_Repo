...
import mmsoctktakeapp.model.view.StkTakeExportVOImpl;
import mmsoctktakeapp.model.view.StkTakeExportVORowImpl;
...
// ---------------------------------------------------------------------
public class MMStkTakeAMImpl extends ApplicationModuleImpl implements MMStkTakeAM //, WFApprove
{
    /**
    ...
    public void setitemforonewarehouse(String itm, String wh) {
        ...
        if (r.length > 0) {
            ...
            if (count == 0) {
                ...
                //  String msg="Item for selected Warehouse is already added to this Slip";
                FacesMessage msg = new FacesMessage(resolvElDCMsg("#{bundle['MSG.2330']}").toString());
                msg.setSeverity(FacesMessage.SEVERITY_ERROR);
                FacesContext ctx = FacesContext.getCurrentInstance();
                ctx.addMessage(null, msg);
            ...
            FacesMessage msg = new FacesMessage(resolvElDCMsg("#{bundle['MSG.2531']}").toString());
            msg.setSeverity(FacesMessage.SEVERITY_ERROR);
            FacesContext ctx = FacesContext.getCurrentInstance();
            ctx.addMessage(null, msg);
    ...
    public void setItemForAllWh(String itm) {
        ...
        if (r.length > 0) {
           ...
           new FacesMessage(resolvElDCMsg("This Item for all warehouse already added to slip.").toString());
            ...
            FacesMessage msg = new FacesMessage(resolvElDCMsg("#{bundle['MSG.2532']}").toString());
            msg.setSeverity(FacesMessage.SEVERITY_ERROR);
            FacesContext ctx = FacesContext.getCurrentInstance();
            ctx.addMessage(null, msg);
        }
    ...
    public void updateAuth() {
        System.out.println(" innnn the updateAuth");
        ...
        if ("A".equals(retVal)) {
            ...
            System.out.println("cldid::" + CldId + "slocid::" + SlocId + "orgid::" + OrgId + "docid::" + DocId +
                               "fyid::" + FyId + "usrid::" + UsrId);
            ...
            try {
                VoucherId = callStoredFunction(Types.VARCHAR, "MM.MM_GET_RCPT_PROV_GL(?,?,?,?,?,?)", new Object[] {
            ...
            if (VoucherId != null) {
                System.out.println("Voucher Id :" + VoucherId.toString());
                // String msg ="Stock Adjusted with Voucher Id :";
                String msg1 = resolvElDCMsg("#{bundle['MSG.2334']}").toString();

                FacesMessage message = new FacesMessage(msg1 + VoucherId.toString());
            ...
            if (VoucherId == null) {
                {
                    String stkNum = (String) getMmStkTake1().getCurrentRow().getAttribute("StkTakeNo");
                    // String msg ="No Voucher is Passed against";
                    String msg1 = resolvElDCMsg("#{bundle['MSG.2335']}").toString();
                    FacesMessage message = new FacesMessage(msg1 + stkNum);
                    message.setSeverity(FacesMessage.SEVERITY_INFO);
                    FacesContext fc = FacesContext.getCurrentInstance();
                    fc.addMessage(null, message);
    ...
    //
    //        RequestContext.getCurrentInstance().getPageFlowScope().put("WF_RET_VALUE", wfReturn);
    //
    //        ViewObjectImpl stktake = getMmStkTake1();
    //        Row[] rw = stktake.getFilteredRows("DocId", txn);
    //        Row r = rw[0];
    //        stktake.setCurrentRow(r);
    ...
    //        String appModuleName = "mmsoctktakeapp.model.services.MMStkTakeAM";
    //        String appModuleConfig = "MMStkTakeAMLocal";
    //        return (MMStkTakeAMImpl) Configuration.createRootApplicationModule(appModuleName, appModuleConfig);
    //    }

    public boolean updatebcFromCsv(String query, Number pqty, Number rqty, Number sqty, String unit) {
        MmStkTakeBcVOImpl bc1 = this.getMmStkTakeBc1();
        RowQualifier rq = new RowQualifier(bc1);
        rq.setWhereClause(query);
        Row[] filteredRows = bc1.getFilteredRows(rq);
        if (filteredRows.length > 0) {
            //update
            filteredRows[0].setAttribute("PhyQty", pqty);
            filteredRows[0].setAttribute("RwkQty", rqty);
            filteredRows[0].setAttribute("ScrpQty", sqty);
        } else {
            // insert
            Row createRow = bc1.createRow();
            bc1.insertRow(createRow);
            ...
            for (String s : split) {
                String[] split_2 = s.split("=");
                int i = split_2[1].indexOf("'");
                int j = split_2[1].lastIndexOf("'");
                System.out.println("data: " + split_2[0].trim() + " -- " +
                                   ((split_2[1].contains("'")) ? split_2[1].substring(i + 1, j) : split_2[1].trim()));
                createRow.setAttribute(split_2[0].trim(),
                                       (split_2[1].contains("'")) ? split_2[1].substring(i + 1, j) : split_2[1].trim());
            }
            createRow.setAttribute("ItmUom", unit);
            createRow.setAttribute("PhyQty", pqty);
            createRow.setAttribute("RwkQty", rqty);
            createRow.setAttribute("ScrpQty", sqty);
        }
        return false;
    }

    /**
     * Container's getter for StkTakeExport1.
     * @return StkTakeExport1
     */
    public StkTakeExportVOImpl getStkTakeExport1() {
        return (StkTakeExportVOImpl) findViewObject("StkTakeExport1");
    }

    public String filterStkTakeEprtVO() {
        Row currRow = this.getMmStkTake1().getCurrentRow();
        String docId = (String) currRow.getAttribute("DocId");
        String cldId = (String) currRow.getAttribute("CldId");
        Integer slocId = (Integer) currRow.getAttribute("SlocId");
        String orgId = (String) currRow.getAttribute("OrgId");
        System.out.println("cur row:  " + this.getMmStkTakeItm1().getCurrentRow());
        String whId = (String) this.getMmStkTakeItm1().getCurrentRow().getAttribute("WhId");
        ViewObjectImpl vo = this.getStkTakeExport1();
        ...
        vo.executeQuery();
        System.out.println("---12--------execute export vo--- " + vo.getRangeSize() + " " + docId + " " + whId);
        String stmt =
            "CldId" + "," +
            "SlocId,HoOrgId,OrgId,DocId,WhId,ItmId,SrNo,ItmUom,PhysicalQty,ReworkableQty,ScrapQty,LotId,BinId,Organization,Warehouse,Item,Unit,Bin" +
            "\n";

        RowSetIterator rsi = vo.createRowSetIterator(null);
        while (rsi.hasNext()) {
            StkTakeExportVORowImpl rr = (StkTakeExportVORowImpl) rsi.next();
            stmt +=
                "@" + rr.getCldId() + "," + rr.getSlocId() + ",@" + rr.getHoOrgId() + ",@" + rr.getOrgId() + "," +
                rr.getDocId() + "," + rr.getWhId() + "," + rr.getItmId() + "," +
                (rr.getSrNo() == null ? 0 : rr.getSrNo()) + "," + rr.getItmUom() + "," +
                (rr.getPhyQty() == null ? 0 : rr.getPhyQty()) + "," + (rr.getRwkQty() == null ? 0 : rr.getRwkQty()) +
                "," + (rr.getScrpQty() == null ? 0 : rr.getScrpQty()) + "," +
                (rr.getLotId() == null ? 0 : rr.getLotId()) + "," + (rr.getBinId() == null ? 0 : rr.getBinId()) + "," +

                rr.getOrgDesc() + "," + rr.getWhNm() + "," + rr.getItmDesc() + "," + rr.getUomDesc() + "," +
                (rr.getBinNm() == null ? 0 : rr.getBinNm());
            stmt += "\n";
    ...
    /**
     * Container's getter for DispWhNm1.
     * @return DispWhNm1
     */
    public DispWhNmVOImpl getDispWhNm1() {
        return (DispWhNmVOImpl) findViewObject("DispWhNm1");
    }
}
