...
public class MmMtlRcptBinVOImpl extends ViewObjectImpl implements MmMtlRcptBinVO {
    ...
    public String isBinQtyValid(String lotNo,String binId,Number obj){
       ...
       //  String binId=this.getCurrentRow().getAttribute("BinId").toString();
         ...
         if(r.length>0){
            lotqty= (Number)((Number)r[0].getAttribute("LotQty")).round(getGlblRoundQtyDigit());
         }
        ...
        //    and BinId='"+binId+"'
         ...
         for(Row lrw:filRw){
            if(lrw!=this.getCurrentRow()){
                  Number binqty=(Number)((Number)lrw.getAttribute("BinQtyBs")).round(getGlblRoundQtyDigit());
                  if(binqty==null){
                      binqty=new Number(0);
                  }
         ...
         sumqty=sumqty.add((Number)obj.round(getGlblRoundQtyDigit()));
         System.out.println("SUMQTY="+sumqty+"---- LOTQTY="+lotqty);
         if(sumqty.compareTo((Number)lotqty.round(getGlblRoundQtyDigit()))==1){
             stat="Y";
         }
        }
        return stat;
    ...
    /**
     * Method to check if bin quantity is valid.
     * Type -  R - Rejected , W - Reworkable
     * @return Y-Invalid Bin Quantity
     *         N- Valid
     * */
    public String isBinQtyRejRwkValid(String lotNo,String binId,Number obj ,String type){
        MMReceiptMtlAMImpl am=(MMReceiptMtlAMImpl)this.getApplicationModule();
        Number lotqty=new Number(0);
        String stat="N";
        if(this.getCurrentRow().getAttribute("LotId")!=null || this.getCurrentRow().getAttribute("BinId")!=null){
       ...
       //  String binId=this.getCurrentRow().getAttribute("BinId").toString();
         ...
         if(r.length>0){
             if("R".equalsIgnoreCase(type)){
               lotqty= (Number)((Number)r[0].getAttribute("RejQtyBs")).round(getGlblRoundQtyDigit());
             }else if("W".equalsIgnoreCase(type)){
                 lotqty= (Number)((Number)r[0].getAttribute("RwkQtyBs")).round(getGlblRoundQtyDigit());
             }
         }
         System.out.println(type+" type and quantity  "+lotqty);
         RowQualifier rq=new RowQualifier(this);
         rq.setWhereClause("LotId='"+lotNo+"'");
        //    and BinId='"+binId+"'
         Row[] filRw=this.getFilteredRows(rq);
         ...
         Number sumqty=new Number(0);
    if("R".equalsIgnoreCase(type)){
        System.out.println(" bin check tyep R");
         for(Row lrw:filRw){
            if(lrw!=this.getCurrentRow()){
                    Number binqty=(Number)((Number)lrw.getAttribute("RejQtySp")).round(getGlblRoundQtyDigit());
                  if(binqty==null){
                      binqty=new Number(0);
                  }
                  sumqty=sumqty.add(binqty);
            ...
            System.out.println(" bin check tyep W");
         for(Row lrw:filRw){
            if(lrw!=this.getCurrentRow()){
                    Number binqty=(Number)((Number)lrw.getAttribute("RwkQtyBs")).round(getGlblRoundQtyDigit());
                  if(binqty==null){
                      binqty=new Number(0);
                  }
                  sumqty=sumqty.add(binqty);
    ...
    public void setValBinQtyBs(Number binQty){
        ...
        Number fctr=(Number)((Number)curR.getAttribute("UomConvFctr")).round(getGlblRoundQtyDigit());
     //   Row r=this.getCurrentRow();
        if(binQty==null){
            binQty=new Number(0);
        }
        
        Number mult=(Number)(((Number)binQty.round(getGlblRoundQtyDigit())).divide(fctr)).round(getGlblRoundQtyDigit());
    ...
    /**
     *
     * @param binQty Rejected or Reworkable 
     * @param type  R - Rejected and W - Reworkable
     */
    
    public void setValBinQtyRejRwkBs(Number binQty ,String type){
        //Twist here. We are setting BinQtyBs to BinQty.Opposite to Function Name. As on Page it is BinQtyBs.
        MMReceiptMtlAMImpl am=(MMReceiptMtlAMImpl)this.getApplicationModule();
        Row curR=am.getMmMtlRcptItm1().getCurrentRow();
        Number fctr=(Number)((Number)curR.getAttribute("UomConvFctr")).round(getGlblRoundQtyDigit());
     //   Row r=this.getCurrentRow();
        ...
        Number mult=(Number)(((Number)binQty.round(getGlblRoundQtyDigit())).divide(fctr)).round(getGlblRoundQtyDigit());
        if("R".equalsIgnoreCase(type)){
        System.out.println(type+" binQtyreject::: vo imple  "+binQty+"  mult  "+mult);
        this.getCurrentRow().setAttribute("RejQtySp",mult);
        }
        else if("W".equalsIgnoreCase(type)){
            System.out.println(type+"binQty Rework::: vo imple  "+binQty+"  mult  "+mult);
            this.getCurrentRow().setAttribute("RwkQtySp",mult);
        }
