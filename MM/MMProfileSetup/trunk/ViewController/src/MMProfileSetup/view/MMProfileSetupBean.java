...
public class MMProfileSetupBean {
    ...
    private String autoItmDisable = "false";

    public void setAutoItmDisable(String autoItmDisable) {
        this.autoItmDisable = autoItmDisable;
    }

    public String getAutoItmDisable() {
        return autoItmDisable;
    }
    ...
    public void addPrfActionListener(ActionEvent actionEvent) {
        ...
        if (result == -2) {
            autoItmDisable = "false";
            //            Profile For is not Freezed. So first freeze profile for [}
            String s1 = ADFModelUtils.resolvRsrc("MSG.2017").toString();
            String s2 = ADFModelUtils.resolvRsrc("MSG.2020").toString();
            String s3 = ADFModelUtils.resolvRsrc("MSG.2022").toString();
            FacesMessage message = new FacesMessage(s1 + "" + HoName + "" + s2 + HoName + s3);
        ...
        if (result == -1) {
            autoItmDisable = "false";
            //            Please First Create The Profile For [}
            String s1 = ADFModelUtils.resolvRsrc("MSG.2025").toString();
            String s2 = ADFModelUtils.resolvRsrc("MSG.2022").toString();
            FacesMessage message = new FacesMessage(s1 + HoName + s2);
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
            FacesContext fc = FacesContext.getCurrentInstance();
            fc.addMessage(null, message);
    ...
    public void editPrfActionListene(ActionEvent actionEvent) {
        // Add event code here...
        System.out.println("this is first edit button");
        flag = "false";
        flag1 = "Y";
        freezsmode = "S";
        this.mode = "E";
    ...
    public void editSavePrfActionListene(ActionEvent actionEvent) {
        ...
        if (getFreezeBind().getValue().toString().equalsIgnoreCase("true")) {
            ...
            String msg2 = ADFModelUtils.resolvRsrc("MSG.91").toString();
    ...
    public void freezeDialogListener(DialogEvent dialogEvent) {
        if (dialogEvent.getOutcome().name().equalsIgnoreCase("yes")) {
            ...
            if (freezeBind.getValue().equals(true) && whbind.getValue().equals(false)) {
                System.out.println("OrgId:" + OrgId + "CldId:" + CldId + "HoOrg:" + HoOrgId + "SlocId:" + SlocId +
                                   "UsrId:" + UsrId);
                Integer result = (Integer) (callStoredFunction(Types.INTEGER, "APP.FN_GEN_AUTO_WH (?,?,?,?,?)", new Object[] {
                                                               CldId, SlocId, HoOrgId, OrgId, UsrId
                }));
            ...
            String msg2 = ADFModelUtils.resolvRsrc("MSG.1224").toString();
            FacesMessage message2 = new FacesMessage(msg2);
            message2.setSeverity(FacesMessage.SEVERITY_INFO);
            FacesContext fc = FacesContext.getCurrentInstance();
            fc.addMessage(null, message2);
    ...
    public void mmProfileValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (object != null) {
            ...
            if (number.compareTo(new oracle.jbo.domain.Number(0)) <=
                0) {
                //                Quantity should be greater than zero.
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                              ADFModelUtils.resolvRsrc("MSG.474").toString(), null));
            }


            if (!isPrecisionValid(26, 6, number)) {
                //                Invalid Precision (26,6).
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                              ADFModelUtils.resolvRsrc("MSG.57").toString(), null));
            }
    ...
    public void DisableEditActionListener(ActionEvent actionEvent) {
        // Add event code here...

        this.mode1 = "E";
        System.out.println("this disableeditaction listener " + mode1);
        ...
        //        //this.mode="E";
        //        //this.mode2="E";
        //        System.out.println("value of mode2 is"+mode2);
    }
    ...
    public void DisableSaveAction(ActionEvent actionEvent) {
        // Add event code here...
        System.out.println(" in the second save method");
        this.mode1 = "D";
        BindingContainer bindings = getBindings();
        OperationBinding operationBinding = bindings.getOperationBinding("Commit");
        operationBinding.execute();
        ...
        //        String msg2 = resolvElDCMsg("#{bundle['']}").toString();
        String msg2 = ADFModelUtils.resolvRsrc("MSG.91").toString();
        FacesMessage message2 = new FacesMessage(msg2);
        message2.setSeverity(FacesMessage.SEVERITY_INFO);
        FacesContext fc = FacesContext.getCurrentInstance();
    ...
    public String getHoOrgFeeze() {
        String chk = "N";
        OperationBinding op1 = getBindings().getOperationBinding("getHoOrgId");
        op1.execute();
        if (op1.getResult() != null) {
            chk = op1.getResult().toString();
        }
        System.out.println("getHoOrgFeeze " + chk);
        return chk;
