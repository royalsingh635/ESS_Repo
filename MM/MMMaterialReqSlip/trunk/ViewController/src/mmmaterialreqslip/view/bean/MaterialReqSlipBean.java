...
public class MaterialReqSlipBean {
    ...
    private RichInputListOfValues customerNameBind;
    private RichPopup wfPopUp;
    ...
    public void addItemActionListener(ActionEvent actionEvent) {
        ...
        //checkRequired
        OperationBinding checkRqrd = getBindings().getOperationBinding("checkRequired");
        checkRqrd.execute();
        String rres = checkRqrd.getResult().toString();
        System.out.println("Result is=" + rres);
        if (rres.length() > 0) {
            if (rres.equalsIgnoreCase("R")) {
                System.out.println("results in bean is ===" + rres);
                FacesMessage message = new FacesMessage("Requirement Area required");
                message.setSeverity(FacesMessage.SEVERITY_ERROR);
                FacesContext fc = FacesContext.getCurrentInstance();
                fc.addMessage(rqmtBind.getClientId(), message);
                return;
            } else if (rres.equalsIgnoreCase("E")) {
             //   FacesMessage message = new FacesMessage("Customer required");
             FacesMessage message = new FacesMessage("Entity required");
                message.setSeverity(FacesMessage.SEVERITY_ERROR);
                FacesContext fc = FacesContext.getCurrentInstance();
                fc.addMessage(customerNameBind.getClientId(), message);
                return;
            }
    ...
    public void cancelMRSValueChange(ValueChangeEvent valueChangeEvent) {
        ...
        //
        OperationBinding op2 = bindings.getOperationBinding("cancelMRS");
        op2.execute();
        Integer cancelChkVal = (Integer) op2.getResult();
        System.out.println(" cancel return value is -" + cancelChkVal);
        /*   if(cancelChkVal==-2){
          FacesMessage message = new FacesMessage("You can not cancel this MRS.");
          message.setSeverity(FacesMessage.SEVERITY_ERROR);
          FacesContext fc = FacesContext.getCurrentInstance();
          fc.addMessage(cancelMRSBinding.getClientId(), message);
        ...
        System.out.println("calling inside ben vcl cancle "+cancelMRSBinding.getValue().toString()+ cancelChkVal);
      }
   */System.out.println(" cancel value --" + cancelMRSBinding.getValue().toString());
        //
        if (status == 341 || cancelChkVal == -2) {
            FacesMessage message = new FacesMessage("Cancelled MRS can not be changed");
            message.setSeverity(FacesMessage.SEVERITY_ERROR);
            FacesContext fc = FacesContext.getCurrentInstance();
            fc.addMessage(dateBind.getClientId(), message);
            FacesMessage message1 = new FacesMessage("You can not cancel this MRS.");
            message1.setSeverity(FacesMessage.SEVERITY_ERROR);
            FacesContext fc1 = FacesContext.getCurrentInstance();
            fc1.addMessage(cancelMRSBinding.getClientId(), message1);
            cancelMRSBinding.setValue(false);
            AdfFacesContext.getCurrentInstance().addPartialTarget(cancelMRSBinding);
            AdfFacesContext.getCurrentInstance().addPartialTarget(cancleColumn);
            System.out.println("calling inside ben vcl cancle " + cancelMRSBinding.getValue().toString() +
                               cancelChkVal);
            ...
            if (chkPr.getErrors().size() == 0)
                if (chkPr.getResult() != null) {
                    ...
                    if (rp.getResult() != null) {
                        if(rp.getResult().toString().equalsIgnoreCase("N")){
                            FacesMessage message2 = new FacesMessage("Something went wrong, Please contact ESS.");
                            message2.setSeverity(FacesMessage.SEVERITY_INFO);
                            FacesContext fc = FacesContext.getCurrentInstance();
                            fc.addMessage(null, message2);
                        }else{
    ...
    public void itmQtyValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (object != null) {
            ...
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Please Enter Quantity.", null));
    }
    ...
    public void addData(ActionEvent actionEvent) {
        OperationBinding op = getBindings().getOperationBinding("GenerateMrsDocument");
        // op.getParamsMap().put("itmname", valueChangeEvent.getNewValue());
        op.execute();
    }
    ...
    public void docTypeVCL(ValueChangeEvent valueChangeEvent) {

        if (valueChangeEvent.getNewValue() != null) {
            OperationBinding binding = getBindings().getOperationBinding("resetAllValue");
            binding.execute();
        }
    }
    ...
    public void setCustomerNameBind(RichInputListOfValues customerNameBind) {
        this.customerNameBind = customerNameBind;
    }

    public RichInputListOfValues getCustomerNameBind() {
        return customerNameBind;
    }

    public void setWfPopUp(RichPopup wfPopUp) {
        this.wfPopUp = wfPopUp;
    }

    public RichPopup getWfPopUp() {
        //showPopup(wfPopUp, true);
        return wfPopUp;
    }

    public void wFpopUpDialogListener(DialogEvent dialogEvent) {
        // Add event code here...
    }

    public void wfServicesAL(ActionEvent actionEvent) {
        // Add event code here...
        showPopup(wfPopUp, true);
    }
}
