...
public class MmSupplierPerfEvalBean implements Serializable {
  ...
  //  private RichCommandButton saveButton;
   ...
   // private RichCommandButton createButton;
    ...
    private Boolean isReEval=false;
    private RichLink cancelButton;
    private RichLink reEvalLink;

    public void setIsReEval(Boolean isReEval) {
        this.isReEval = isReEval;
    }

    public Boolean getIsReEval() {
        return isReEval;
    }
...
System.out.println("populateData===========");
    ...
    public String SaveAction() {
        ...
        int count = 0;
            ...
            System.out.println("Estimated RowCount is "+v.getEstimatedRowCount());
            if (v.getEstimatedRowCount() > 0) {
             ...
             if(setId!=null){
                   ...
                   if(countNew>0){
                       ...
                       FacesMessage msg = new FacesMessage("The Evaluation is Rollback because Another User Evaluate Same Supplier and Same Parameter Set or Different Parameter Set.");
                       msg.setSeverity(FacesMessage.SEVERITY_WARN);
                       FacesContext ctx = FacesContext.getCurrentInstance();
                       ctx.addMessage(null, msg);
                       return null;
                ...
                /**
                 * Commented By Mayank On 28-04-2015 beacuse this condition is never getting executed
                 * 
                 * add Commit if revalation has been made
                 * */
...
//            createOB.execute();
            ...
            createexe.execute();
            createexe = executeBinding("Execute1");
            createexe.execute();
            // suppliertableBind.setRowSelection(RichTable.ROW_SELECTION_SINGLE);
            am.getMmEoPerfRest1().executeQuery();
            this.setSetupVis("true");
            if (createexe.getErrors().isEmpty()) {
                ...
                reEvaluateButton = getreEvaluateButtonVal();
                this.setMode("");
                this.setIsReEval(true);
                AdfFacesContext.getCurrentInstance().addPartialTarget(this.getSaveButton());
                AdfFacesContext.getCurrentInstance().addPartialTarget(this.getCancelButton());
    ...
    public void cancelProfile(ActionEvent e){
        
       Key parentKey=((DCIteratorBinding)this.getBindings().get("AppEoPrfIterator")).getCurrentRow().getKey();
        
        OperationBinding createOB1 = executeBinding("Rollback");
        createOB1.execute();
        
        ((DCIteratorBinding)this.getBindings().get("AppEoPrfIterator")).setCurrentRowWithKey(parentKey.toStringFormat(true));
        this.setIsReEval(false);
        this.setMode("V");
        this.setReadOnlyField(true); 
    ...
    public Integer getCount(){
        ...
        OperationBinding op=this.getBindings().getOperationBinding("getCount");
        op.execute();
        if(op.getErrors().size()>0){
            System.out.println("Error are "+op.getErrors());
        }
    ...
    public void setCancelButton(RichLink cancelButton) {
        this.cancelButton = cancelButton;
    }

    public RichLink getCancelButton() {
        return cancelButton;
    }

    public void setReEvalLink(RichLink reEvalLink) {
        this.reEvalLink = reEvalLink;
    }

    public RichLink getReEvalLink() {
        return reEvalLink;
    }
}
