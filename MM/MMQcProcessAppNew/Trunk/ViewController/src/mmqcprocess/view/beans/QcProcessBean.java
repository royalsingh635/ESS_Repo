...
public class QcProcessBean {
    ...
    public String saveAction() {
        ...
        if ("Y".equalsIgnoreCase(chekUpdate)) {
            ...
            if ("Y".equalsIgnoreCase(chekType)) {
                ...
                }  else if (checkForCheck.compareTo(new Integer(884)) == 0) {
                    OperationBinding updateRcpt = getBindings().getOperationBinding("updateScReceipt");
                    updateRcpt.execute();
                    if (updateRcpt.getResult() != null) {
                        String retVal = updateRcpt.getResult().toString();
                        loginfo.info("retVal--------" + retVal);
    ...
    public void genereateDispId(){
        OperationBinding callNoOp = getBindings().getOperationBinding("genQcProcessNo");
        callNoOp.getParamsMap().put("SlocId", EbizParams.GLBL_APP_SERV_LOC());
        callNoOp.getParamsMap().put("CldId", EbizParams.GLBL_APP_CLD_ID());
        callNoOp.getParamsMap().put("HoOrgId", EbizParams.GLBL_HO_ORG_ID());
        callNoOp.getParamsMap().put("OrgId", EbizParams.GLBL_APP_USR_ORG());
        callNoOp.getParamsMap().put("TableName","MM$MTL$QC");
