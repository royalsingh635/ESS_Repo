...
public class SCReceiptCreateBean {
    ...
    public void showError(String clientId,String header,String message){
        FacesContext.getCurrentInstance().addMessage(clientId, new FacesMessage(FacesMessage.SEVERITY_ERROR,header==null?ADFModelUtils.resolvRsrc("MSG.2151"):header,message));
    }
    ...
    public void editDoc(ActionEvent actionEvent) {
        ...
        if(wf==null){
            String msg1 = ADFModelUtils.resolvRsrc("MSG.2926");    // Set Up Error
            String msg = ADFModelUtils.resolvRsrc("MSG.2927");  // Work FLow is not defined for this document
            ADFModelUtils.showFormattedFacesMessage(msg1, msg, FacesMessage.SEVERITY_ERROR);
           return ; 
        }
        ...
        if(result==-1|| result.equals(EbizParams.GLBL_APP_USR())){
            ...
            String msg = ADFModelUtils.resolvRsrc("MSG.2627");  //Document is pending at other user
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,ADFModelUtils.resolvRsrc("MSG.2151"),msg)); 
        }
    ...
    public void save(ActionEvent actionEvent) {
       ...
       // save Data
        ...
        String msg1 = ADFModelUtils.resolvRsrc("MSG.2159"); //Successfully
        String msg = ADFModelUtils.resolvRsrc("MSG.2670");//Record Save Successfully
        ADFModelUtils.showFormattedFacesMessage(msg1, msg, FacesMessage.SEVERITY_INFO);
    ...
    public String saveAndForward() {
        ...
        if(obj==null){
            this.showError(null, null,ADFModelUtils.resolvRsrc("MSG.2936"));  //Error while validating
            return null;
        }
      ...
      //  System.out.println("After validating");
        ...
        Object wf=op.getResult();
        if(wf==null){
            String msg1 = ADFModelUtils.resolvRsrc("MSG.2926");//Ser Up Errorr
            String msg = ADFModelUtils.resolvRsrc("MSG.2927"); // Work FLow is not defined for this document
            ADFModelUtils.showFormattedFacesMessage(msg1, msg, FacesMessage.SEVERITY_ERROR);
           return null; 
        }
    ...
    public void ApplyTaxForAll(ActionEvent actionEvent) {
        ...
        if(o==null || o.toString().trim().length()==0){
            String msg = ADFModelUtils.resolvRsrc("MSG.2928"); // Please select a tax rule
            FacesContext.getCurrentInstance().addMessage(this.getGlobalTaxBind().getClientId(), new FacesMessage(FacesMessage.SEVERITY_ERROR,ADFModelUtils.resolvRsrc("MSG.2151"),msg));
        }else{
    ...
    public void validateTotqty(FacesContext facesContext, UIComponent uIComponent, Object object) {
        if(object==null||object.toString().trim().length()==0){
            String msg = ADFModelUtils.resolvRsrc("MSG.2929");  //This can not be blank
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,ADFModelUtils.resolvRsrc("MSG.2151"),msg));
        }else{
            ...
            if(value.compareTo(new Number(0))==1){
                ...
                if(message.equalsIgnoreCase("Y")){
                    ...
                    throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,ADFModelUtils.resolvRsrc("MSG.2151"),message));
                }
            }else{
                String msg = ADFModelUtils.resolvRsrc("MSG.2930");  //This can not be zero and negative
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,ADFModelUtils.resolvRsrc("MSG.2151"),msg));
            }
        }
        ...
        if(object==null||object.toString().trim().length()==0){
            ...
            if(value.compareTo(new Number(0))!=-1){
                ...
                if(message.equalsIgnoreCase("Y")){
                   ...
                   String msg = ADFModelUtils.resolvRsrc("MSG.2933");   //This can not be negative
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,ADFModelUtils.resolvRsrc("MSG.2151"),msg));
            }
        }
    ...
    private String validate(String att,Number value){
        ...
        if(penAmt==null){
            ...
            if(tot.compareTo(totRcpt)<0){
                message=ADFModelUtils.resolvRsrc("MSG.2975");   // Total Receipt quantity can not be more than Pending quantity 
                return message;
            }
            
            if(((rej.add(rwk)).compareTo(totRcpt))>0){
                message=ADFModelUtils.resolvRsrc("MSG.2976"); // Sum of rejected,rewokable quantity can not be more than total receipt quantity  
                return message;
            }
    ...
    public void validateBinQty(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if(object==null || object.toString().trim().length()==0){
            ...
            if(result.equalsIgnoreCase("Y")){
                           ...
                           String msg = ADFModelUtils.resolvRsrc("MSG.2935");          // value can not be more than
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,ADFModelUtils.resolvRsrc("MSG.2151"),msg+" : "+result));
            }
        }
    ...
    public String validateHeader(){
        String result="Y";
        if(this.getEntityBind().getValue()==null||this.getEntityBind().getValue().toString().trim().length()==0){
            result="N";
            this.showError(this.getEntityBind().getClientId(),null ,ADFModelUtils.resolvRsrc("MSG.2409"));  // Please make a selection
            return result;
        }
        if(this.getWareHouseBind().getValue()==null||this.getWareHouseBind().getValue().toString().length()==0){
            result="N";
            this.showError(this.getWareHouseBind().getClientId(),null ,ADFModelUtils.resolvRsrc("MSG.2409"));  // Please make a selection
            return result;
        }
        
        if(this.getCurrBind().getValue()==null||this.getCurrBind().getValue().toString().length()==0){
            result="N";
            this.showError(this.getCurrBind().getClientId(),null ,ADFModelUtils.resolvRsrc("MSG.2409"));  // Please make a selection
            return result;
        }
        
        if(this.getProjectId().getValue()==null||this.getProjectId().getValue().toString().length()==0){
            result="N";
            this.showError(this.getProjectId().getClientId(),null ,ADFModelUtils.resolvRsrc("MSG.2409"));  // Please make a selection
            return result;
        }
    ...
    public Boolean getSerial(){
        OperationBinding op=ADFBeanUtils.findOperation("getSearializedDetail");
        op.getParamsMap().put("par", "V");
        op.execute();
       // System.out.println("Serial is "+(Boolean) op.getResult());
        return (Boolean) op.getResult();
    }
}
