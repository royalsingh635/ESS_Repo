    ...
    public Boolean validateInput() {
        ...
        if (row.getDocIdSrcTrans() != null &&
            ...
            if (rw.length > 0) {
                ...
                FacesContext.getCurrentInstance().addMessage(null,
                                                             new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error",
                                                                              "This order and delivery No has been already added."));
            } else {
            ...
            FacesContext.getCurrentInstance().addMessage(null,
                                                         new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error",
                                                                          "Doc no and Delivery no is mandatory"));
            result = false;
        }
        return result;
    }
...
//        System.out.println("Type is " + type);
     ...
     //   System.out.println("Row Count is " + this.getLovScoDlvItem1().getRowCount());
        ...
        if (this.getLovScoDlvItem1().getRowCount() > 0) {
            ...
            if (flag.equalsIgnoreCase("P"))
                FacesContext.getCurrentInstance().addMessage(null,
                                                             new FacesMessage(FacesMessage.SEVERITY_WARN, "Wanring",
                                                                              "Some items from oder has no pending quantity"));
        } else {
            FacesContext.getCurrentInstance().addMessage(null,
                                                         new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error",
                                                                          "There is not any item in Order"));
        }
    ...
    /**
     * Container's getter for FkMmScRcptSrnoVL2.
     * @return FkMmScRcptSrnoVL2
     */
    public ViewLinkImpl getFkMmScRcptSrnoVL2() {
        return (ViewLinkImpl) findViewLink("FkMmScRcptSrnoVL2");
    }


    public void addSearial(String srNo, String lot, String bin) {
        if (srNo == null || srNo.trim().length() == 0) {
            ADFModelUtils.showFormattedFacesMessage("Please enter serial number", "Serial Number is a mandatory field",
        ...
        if (rw.length > 0) {
            ADFModelUtils.showFormattedFacesMessage("Duplicate Record found", "Setial No : " + srNo,
                                                    FacesMessage.SEVERITY_ERROR);
            return;
        }
    ...
    public String validateLot() {
        ...
        if (lotList.size() > 0) {
            msg.append("<p>Lot Entry Missing for following Item</p>");
            msg.append("<ul>");
        ...
        if (list.size() > 0) {
            msg.append("<p>Bin quantity mismatch for following Item</p>");
            msg.append("<ul>");
        ...
        if (srList.size() > 0) {
            msg.append("<p>Serial quantity mismatch for following Item</p>");
            msg.append("<ul>");
        ...
        if (msg.toString().trim().length() > 0) {
            result = "N";
            ADFModelUtils.showFormattedFacesMessage("Quantity mismatch for updating stock", msg.toString(),
                                                    FacesMessage.SEVERITY_ERROR);
        }
        return result;
    }
...
//        String vouId= this.getMmScRcpt1().getCurrentRow().getAttribute("GlDispId")==null ? null:this.getMmScRcpt1().getCurrentRow().getAttribute("GlDispId").toString();
       ...
       if(this.getMmScRcpt1().getRowCount()>0){
        ...
        ADFModelUtils.showFormattedFacesMessage("Voucher generated succefully",
                                                "Voucher Id : "+vouId,
                                                FacesMessage.SEVERITY_INFO
                                                );
