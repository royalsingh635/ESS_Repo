...
public class PdcAppAMImpl extends ApplicationModuleImpl {
  ...
  //  String hoorg_id = resolvEl("#{pageFlowScope.GLBL_HO_ORG_ID}");
     ...
     * @return PdcSearch1
     */
    public ViewObjectImpl getPdcSearch1() {
        return (ViewObjectImpl)findViewObject("PdcSearch1");
    }
    ...
    public void postVoucherToGL() {
        ...
        if (row.length > 0) {
            for (Integer i = 0; i < row.length; i++) {
                ...
                gl_vou_no =
                        callStoredProcedure("FIN.proc_translate_to_gl_pdc(?,?,?,?,?,?,?,?,?,?,?,?,?)", new Object[] { cld_id,
                                                                                                                      sloc_id,
            ...
            FacesMessage message = new FacesMessage(resolvElDCMsg("#{bundle['MSG.1318']}").toString());//Please select Instrument for PDC clearing !
