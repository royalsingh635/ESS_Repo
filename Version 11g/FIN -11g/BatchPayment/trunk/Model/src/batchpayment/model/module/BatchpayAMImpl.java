      ...
      /*  callStoredProcedure("FIN.pkg_fin_gl.INS_BATCH_DETAILS(?,?,?,?,?,?,?,?,?,?,?)",
        ...
        */
        ...
        callStoredProcedure("FIN.INS_BATCH_DETAILS(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                            new Object[] {cldId ,SlocID,hoOrgId,OrgID,InstID,CoaId,UserID,flag,CreateDate,amt,param1 ,param2 , param3 ,param4}); 
            ...
            if(sum.getApTdsRuleId() != null) {
                ...
                System.out.println(" parameters are :"+cld_id+" "+ sloc_id +" "+ho_org_id+" "+ org_id+" "+ inst_id+ " "+vou_id+" "+ vou_dt+" "+ slNo+" "+ tds_id+ " "+inst_amt+ " "+curr_id+ " "+user_id+ " "+vou_dt+" " + coa_id);
                    ...
                    try
                    {
                        ...
                        System.out.println("Vlaue of tax -------- is  -----"+tax);
                    ...
                    sum.setApPayInstrmntAmt(sum.getApSumAmtAdj().subtract(tax_amt));
                    this.getDBTransaction().commit();    
                }
                catch(Exception e) {
        ...
        if (wfReturn.equals("A")) {
            ...
            try{
               this.genarateVoucher();
                this.getDBTransaction().commit();
               FacesMessage msg = new FacesMessage(resolvElDCMsg("Record Approved Successfully").toString());
               msg.setSeverity(FacesMessage.SEVERITY_INFO);
               FacesContext.getCurrentInstance().addMessage(null, msg);
            } catch(Exception e){
                    FacesMessage msg = new FacesMessage(resolvElDCMsg("Error in generating Bank Payments").toString());
                    msg.setSeverity(FacesMessage.SEVERITY_ERROR);
                    FacesContext.getCurrentInstance().addMessage(null, msg); 
                }
        } else {
       ...
       System.out.println("current Coa Id for reset "+coaId);
        ...
        try{ 
              callStoredProcedure("FIN.proc_reset_tds_rule_in_batch(?,?,?,?,?,?,?)",
                ...
                this.getApPaySum1().getCurrentRow().setAttribute("ApPayInstrmntAmt",(Number) this.getApPaySum1().getCurrentRow().getAttribute("ApSumAmtAdj"));
