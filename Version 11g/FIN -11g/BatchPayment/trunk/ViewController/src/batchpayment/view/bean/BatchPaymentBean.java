...
public class BatchPaymentBean implements Serializable {
    ...
    public void tdsRuleIdValueChangeList(ValueChangeEvent valueChangeEvent) {
        ...
        /* BatchpayAMImpl am = (BatchpayAMImpl)resolvElDC("BatchpayAMDataControl");
         ViewObjectImpl paySumVO = am.getApPaySum1();
         if(paySumVO.getCurrentRow() != null)
         {
            ApPaySumVORowImpl r1 = (ApPaySumVORowImpl)paySumVO.getCurrentRow();

            ViewObjectImpl sysdateVo = am.getSystemDate();
            RowSetIterator itr = sysdateVo.createRowSetIterator(null);
             SystemDateVORowImpl r2;
            if(itr.hasNext())
                r2 = (SystemDateVORowImpl)itr.next();
            //SystemDateVORowImpl r2 = (SystemDateVORowImpl)sysdateVo.getCurrentRow();
       ...
       // Integer instId = 1;
        Integer tdsId = Integer.parseInt(valueChangeEvent.getNewValue().toString());  // giving null pointer exception
        Integer coaId =  r1.getApCoaId();
        Number amtAdj = new Number(0);
        String vouId = r1.getApPayId();

        Integer slNO = 1;
        Integer currId = 73;
        Date voudt = (Date)Date.getCurrentDate();
      amtAdj = r1.getApSumAmtAdj();

        System.out.println(SlocId +"--"+orgId+"----"+ho_org_id+"---"+cld_id+"---"+vouId+"----"+voudt+"----"+slNO+"-----"+inst_id+"--- "+amtAdj+"---"+currId+"---"+usrId+"----"+voudt+"----"+inst_id+"----"+coaId);
        // BigDecimal tax = (BigDecimal)callStoredFunction(NUMBER, "FIN.pkg_fin_gl.INS_TDS_FOR_BATCH(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        //                    new Object[] {SlocId,orgId,ho_org_id,cld_id, vouId,voudt,slNO,tdsId,amtAdj,currId,usrId,voudt,instId,coaId});

         BigDecimal tax = (BigDecimal)callStoredFunction(NUMBER, "FIN.INS_TDS_FOR_BATCH(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                            new Object[] {cld_id,SlocId,ho_org_id,orgId,inst_id,vouId,voudt,slNO, tdsId,amtAdj,currId,usrId,voudt,coaId});

        System.out.println(tax+"-------------------bigdecimal value");
        Number tds = new Number(tax);
        System.out.println(tds+"---------------tax");
        r1.setApPayTdsAmt(tds);
        Number instAmt = amtAdj.subtract(tds);
        r1.setApPayInstrmntAmt(instAmt);
         */
        valueChangeEvent.getComponent().processUpdates(FacesContext.getCurrentInstance());
  ...
  public void resetTdsLink(ActionEvent actionEvent) {
    ...
    public void newBatchPaymentAddAction(ActionEvent actionEvent) {
        ...
        String check =
            callStoredFunction(Types.VARCHAR, "FIN.FN_IS_CR_EXIST_FOR_BATCH(?,?,?,?,?)", new Object[] { this.getCurrentCld(),
...
callStoredFunction(Types.VARCHAR, "FIN.FN_SAVE_BATCH(?,?,?,?,?,?)", new Object[] { cld_id, sloc_id, ho_org_id, org_id,
    ...
    private void genarateVoucher() {
        ...
        if (impl.getCurrentRow() != null) {
            ...
            try {
                BigDecimal n =
                    (BigDecimal)callStoredFunction(NUMBER, "FIN.fn_bp_batch(?,?,?,?,?,?,?,?,?)", new Object[] { cld_id,
                                                                                                                SlocId,
                                                                                                                ho_org_id,
                                                                                                                orgId,
                                                                                                                inst_id,
                ...
                if (check == 1) {
                    ...
                    setMode("view");
                    RowSetIterator itr = am.getApPaySum1().createRowSetIterator(null);
                    while (itr.hasNext()) {
                        ApPaySumVORowImpl lo = (ApPaySumVORowImpl)itr.next();
                        lo.getLovVouIdVO1().executeQuery();
                    }
                    ...
                    ActionEvent eve = new ActionEvent(forwardbuttonBind);
                    eve.queue();
    ...
    public void validateInstrumentNumber(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        OperationBinding ob=this.getBindings().getOperationBinding("validateInstrument");
        ob.getParamsMap().put("instrumentNumber", object.toString());
        ob.execute();
