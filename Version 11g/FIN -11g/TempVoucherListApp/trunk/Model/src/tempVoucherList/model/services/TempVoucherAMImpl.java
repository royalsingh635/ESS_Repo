...
// ---------------------------------------------------------------------
public class TempVoucherAMImpl extends ApplicationModuleImpl implements TempVoucherAM {
     ...
     * @return Tvou1
     */
    public ViewObjectImpl getTvou1() {
        return (ViewObjectImpl)findViewObject("Tvou1");
    }
     ...
     * @return TvouLinesLnk
     */
    public ViewObjectImpl getTvouLinesLnk() {
        return (ViewObjectImpl)findViewObject("TvouLinesLnk");
    }
    ...
    /**
     * Function to process tax i.e. insert data in to TaxLines according to the selected tax rule using database procedure "PKG_FIN_TVOU.INS_TEMP_TAX_LINE".
     */
    ...
    public String procTaxForHdr(String ruleId) {
        ...
        /** call database function "INS_TEMP_TAX_LINE" to generate tax rule lines according to rule selected in tax rule and amount specified. */
        callStoredProcedure("FIN.INS_TEMP_TAX_LINE(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                            new Object[] { cldId, slocId, hoOrgId, orgId, instId, vouId, tvouDt, slNo, taxRuleId,
                                           exmptFlg, finAmt, currIdsp, userId, tvouDt, op });
    ...
    /**
     * Function to process tds i.e. insert data in to TdsLines according to the selected tds rule using database function "INS_TEMP_TDS_LINE".
     */
    ...
    public String procTdsForHdr(String ruleid) {
     ...
     //   System.out.println("tds function called");
        System.out.println(slocId + " " + orgId + " " + vouId + " " + tvouDt + " " + slNo + " " + tdsRuleId + " " +
                           finAmt + " " + currIdsp + " " + Op + " " + usrId + " " + tvouDt);
        /** call database function "INS_TEMP_TDS_LINE" to generate tds rule lines according to rule selected in tds rule and amount specified. */
        callStoredProcedure("FIN.INS_TEMP_TDS_LINE(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                            new Object[] { cldId, slocId, hoOrgId, orgId, instId, vouId, tvouDt, slNo, tdsRuleId,
                                           finAmt, currIdsp, usrId, tvouDt, Op });
        rit.closeRowSetIterator();
    ...
    public void saveAsGL() {
        ...
        Integer userId = Integer.parseInt(ADFUtil.resolvEl("#{pageFlowScope.GLBL_APP_USR1}").toString());
        String hoOrg_id = ADFUtil.resolvEl("#{pageFlowScope.GLBL_HO_ORG_ID1}").toString();
        String cld_id = ADFUtil.resolvEl("#{pageFlowScope.GLBL_APP_CLD_ID1}").toString();
        String orgId = ADFUtil.resolvEl("#{pageFlowScope.ParamOrgId}").toString();
        Integer slocId = Integer.parseInt(ADFUtil.resolvEl("#{pageFlowScope.GLBL_APP_SERV_LOC1}").toString());
        ...
        callStoredProcedure("FIN.PROC_TRANSLATE_TO_GL(?,?,?,?,?,?,?)",
                            new Object[] { cld_id, slocId, hoOrg_id, orgId, instId, vouId, userId });
        /* String disp_glId =
            (String)callStoredFunctionForGlDisp(VARCHAR, "FIN.FN_GL_DISP_NO(?,?,?,?,?)", new Object[] { slocId, cld_id, orgId,
                                                                                             vouId });
        System.out.println("disp_glId " + disp_glId);
        System.out.println("getMultiOrgFlag ------------------------= "+getMultiOrgFlag()); */
        ...
        /* StringBuilder saveMsg =
            new StringBuilder("<html><body><p><b>Interim Voucher successfully translated to GL with Voucher Id - " +
                              disp_glId + "</b></p>");
    ...
    public void saveAsTemplate(String vouId, String name) {
        ...
        String orgId = ADFUtil.resolvEl("#{pageFlowScope.ParamOrgId}").toString();
        Integer slocId = Integer.parseInt(ADFUtil.resolvEl("#{pageFlowScope.GLBL_APP_SERV_LOC1}").toString());
        Integer usrId = Integer.parseInt(ADFUtil.resolvEl("#{pageFlowScope.GLBL_APP_USR1}").toString());
      /*   System.out.println("VouId-->" + vouId + "hoOrg-->" + hoOrg_id + "cldid--->" + cld_id + "orgId-->" + orgId +
                           "slocId-->" + slocId); */
        callStoredFunction(NUMBER, "FIN.FN_SAVE_AS_TEMPLATE(?,?,?,?,?,?,?,?,?,?,?)",
                           new Object[] { cld_id, slocId, hoOrg_id, orgId, name, vouId, "N", "U", "S", "1", usrId });
    }
    ...
    public void taxConsolidation() {
        ...
        callStoredFunction(NUMBER, "FIN.FN_TVOU_TAX(?,?,?,?,?,?,?,?,?)",
                           new Object[] { cldId, slocId, vouInsId, hoOrgId, orgId, vouId, vouTyp, vouSubTyp,
                                          tvouSpId });
    ...
    public void tdsConsolidation() {
        ...
        if (!vouTyp.equals(14) || !vouTyp.equals(8)) {
            callStoredFunction(NUMBER, "FIN.FN_TVOU_TDS(?,?,?,?,?,?,?,?,?)",
                               new Object[] { cldId, slocId, vouInsId, hoOrgId, orgId, vouId, vouTyp, vouSubTyp,
                                              tvouSpId });
        }
    ...
    public void tvouLineOp() {
        ...
        try {
            callStoredFunction(NUMBER, "FIN.FN_TVOU_LINES_HEADER_LINE_OP(?,?,?,?,?,?)",
                               new Object[] { cldId, slocId, instId, hoOrgId, orgId, vouId });
        } catch (Exception e) {
    ...
    public void tvouOtherCharges() {
        ...
        callStoredFunction(NUMBER, "FIN.FN_TVOU_OC (?,?,?,?,?,?,?)",
                           new Object[] { cldId, slocId, instId, hoOrgId, orgId, vouId, currId });
    ...
    public void tvoudr() {
        ...
        callStoredFunction(NUMBER, "FIN.FN_TVOU_DR (?,?,?,?,?,?)",
                           new Object[] { cldId, slocId, instId, hoOrgId, orgId, vouId });
    ...
    public void generateDocNo() {
        ...
        //  System.out.println("TvouId---->" + vouId + "TvouDt---->" + vouDt + "UserId----->" + usrId);
        try {
            callStoredFunction(VARCHAR, "APP.INS_DISP_DOC_TAB (?,?,?,?,?,?,?,?,?,?)",
                               new Object[] { cldId, slocId, instId, hoOrgId, orgId, vouId, docId, vouType, usrId,
                                              vouDt });
    ...
    /**
     * Function to call a database function to validate temporary voucher.
     * */
    public void validateTempVou() {
        ViewObjectImpl tvouV = getTvou1();
        Row tvouVRow = tvouV.getCurrentRow();
        String vouId = tvouVRow.getAttribute("TvouId").toString();
        callStoredProcedure("FIN.PROC_TVOU_VALIDATIONS(?,?)", new Object[] { vouId, "I" });
        //remove pkg_fin_trial, remove if not required
    }
    ...
    public Integer validateTempVouForGl() {
        ...
        /*  String vouId = getTvou1().getCurrentRow().getAttribute("TvouId").toString();
        String orgId = getTvou1().getCurrentRow().getAttribute("TvouOrgId").toString();
        Integer slocId = Integer.parseInt(getTvou1().getCurrentRow().getAttribute("TvouSlocId").toString());
        String hoorgId = (getTvou1().getCurrentRow().getAttribute("TvouHoOrgId").toString());
        String cldId = getTvou1().getCurrentRow().getAttribute("TvouCldId").toString();
        Integer instId = Integer.parseInt(getTvou1().getCurrentRow().getAttribute("TvouApplInstId").toString()); */

        callStoredProcedure1("FIN.PROC_TVOU_VALIDATIONS(?,?,?,?,?,?,?)",
                             new Object[] { getTvou1().getCurrentRow().getAttribute("TvouCldId"),
     ...
     * @return LovVouCoa
     */
    public ViewObjectImpl getLovVouCoa() {
        return (ViewObjectImpl)findViewObject("LovVouCoa");
    }
    ...
    public String getVoucherStatus(Integer slocId, String orgId, Integer docId, Integer docTypEntId, Integer docTypeId,
        ...
        return (String)callStoredFunction(VARCHAR, "APP.FN_GET_WF_STAT_DM(?,?,?,?,?,?,?,?)",
                                          new Object[] { slocId, orgId, docId, docTypEntId, docTypeId, docTxnId,
                                                         docTxnDt, docUsrId });
    ...
    public void getFromTemplate(String tmplVouId) {
        ...
        callStoredFunction(VARCHAR, "FIN.FN_CREATE_VOUCHER_FROM_TMPL(?,?,?,?,?,?,?,?,?)",
                           new Object[] { cldId, slocId, hoOrgId, orgId, instId, tmplId, vouId, usrId, tvouDt });
    ...
    public String validateChqNo(Integer p_sloc_id, Integer p_bnk_id, Integer p_chq_buk_id, Number p_chq_no) {
        ...
        return (String)callStoredFunction(VARCHAR, "FIN.fn_validate_chq_no (?,?,?,?)",
    ...
    public String getLastChqNo(Integer p_sloc_id, Integer p_bnk_id, String p_search_mode, Integer p_chq_buk_id) {
        ...
        String chqSlNO =
            callStoredFunction(VARCHAR, "fin.fn_get_chq_no (?,?,?,?)", new Object[] { p_sloc_id, p_bnk_id, p_search_mode,
    ...
    public String updateChqNo(Integer p_sloc_id, Integer p_bnk_id, Integer p_chq_buk_id, Number p_chq_no,
        ...
        return (String)callStoredFunction(VARCHAR, "fin.fn_upd_chq_no_data (?,?,?,?,?,?,?,?,?,?,?,?,?)",
    ...
    public Integer getTvouLnSn(String VouId) {
        ...
        return Integer.parseInt(callStoredFunction(NUMBER, "FIN.FN_GET_MAX_SLNO_TVOU(?)",
                                                   new Object[] { VouId }).toString());
        // do be reviewed
    }
    ...
    public void insTvouLineTax(String PARAM_CLD_ID, Integer PARAM_SLOC_ID, Integer PARAM_INST_ID,
        ...
        callStoredFunction(NUMBER, "FIN.FN_INS_TVOULINE_TAX(?,?,?,?,?,?,?,?,?,?)",
                           new Object[] { PARAM_CLD_ID, PARAM_SLOC_ID, PARAM_INST_ID, PARAM_HO_ORG_ID, PARAM_ORG_ID,
                                          PARAM_TVOU_ID, PARAM_TVOU_DT, PARAM_CURR_ID_SP, PARAM_UID, PARAM_USR_DT });
    ...
    public void insTvouLineTds(String PARAM_CLD_ID, Integer PARAM_SLOC_ID, Integer PARAM_INST_ID,
        ...
        callStoredFunction(NUMBER, "FIN.FN_INS_TVOULINE_TDS(?,?,?,?,?,?,?,?,?,?)",
                           new Object[] { PARAM_CLD_ID, PARAM_SLOC_ID, PARAM_INST_ID, PARAM_HO_ORG_ID, PARAM_ORG_ID,
                                          PARAM_TVOU_ID, PARAM_TVOU_DT, PARAM_CURR_ID_SP, PARAM_UID, PARAM_USR_DT });
     ...
     */
    public ViewObjectImpl getTvouSearchVO() {
        return (ViewObjectImpl)findViewObject("TvouSearchVO");
    }
     ...
     * @return
     */
    public Integer checkVoucher_new(String table, String mst_vou_id, String mst_vou_id_val, String adj_vou_id,
                                    String adj_vou_id_val, String radj_vou_id, String radj_vou_id_val) {
        Integer count = null;

        String query =
            "SELECT count(*) from " + table + " WHERE " + mst_vou_id + "= '" + mst_vou_id_val + "' and " + adj_vou_id +
            " = '" + adj_vou_id_val + "' and " + radj_vou_id + " = '" + radj_vou_id_val + "'";
    ...
    public Object getOutstanding(Object cldId, Object slcId, Object hoOrgId, Object orgId, Object instId,
        ...
        return callStoredFunction(NUMERIC, "FIN.FN_OUTSTANDING_AMT(?,?,?,?,?,?,?,?,?)",
                                  new Object[] { cldId, slcId, hoOrgId, orgId, instId, radjVouId, radjGlSlNo,
                                                 radjArapSlNo, vouDt });
    ...
    public BigDecimal getTaxBA(String tvouId, String cldId, String hoOrgId) {
        ...
        return (BigDecimal)callStoredFunction(NUMERIC, "FN_GET_TAX_BA(?,?,?,?,?,?)",
                                              new Object[] { cldId, slocId, hoOrgId, orgId, instId, vouId });
    ...
    public BigDecimal getTdsBA(String tvouId, String cldId, String hoOrgId) {
        ...
        return (BigDecimal)callStoredFunction(NUMERIC, "FN_GET_TDS_BA(?,?,?,?,?,?)",
                                              new Object[] { cldId, slocId, hoOrgId, orgId, instId, vouId });
    }
     ...
     */
    public ViewObjectImpl getLovLatestCurr1() {
        return (ViewObjectImpl)findViewObject("LovLatestCurr1");
    }
    ...
    public void billDetail() {
        try {
            ...
            Integer instId = 1;
            String vouId = tvouVRow.getAttribute("TvouId").toString();
            // String currIdSpLLine = tvouRow.getAttribute("TvouCurrIdSp").toString();

            //Integer currId = Integer.parseInt(tvouVRow.getAttribute("TvouCurrIdSp").toString());
        //    System.out.println("VouId-->" + vouId + "hoOrg-->" + hoOrgId + "cldid-->" + cldId);
            callStoredFunction(NUMBER, "fin.FN_TVOU_BILL_DTL (?,?,?,?,?,?)",
                               new Object[] { cldId, slocId, hoOrgId, orgId, instId, vouId }); 
    ...
    public String procTdsForHdrBill(String ruleid) {
        ...
        //Based on this flag either rows in tds_rule_lines are deleted-inserted or deleted.
        //I for deleted-inserted
        //D for only delete
        String Op = "I"; //default value is I
        ...
        while (rit.hasNext()) {
            ...
            if (lineRow.getAttribute("TvouTdsFlg") != null) {
                ...
                if (lineRow.getAttribute("TvouTdsFlg").equals("Y") && lineRow.getAttribute("TvouCoaId") != null) {
                    ...
                    //total taxabale amount is calculated.
                    as = as.add((Number)(lineRow.getAttribute("TvouAmtSp")));

                   /*  System.out.println("function called" + slocId + " " + orgId + " " + vouId + " " + tvouDt + " " +
                                       billSlNo + " " + ruleid + " " + finAmt + " " + currIdsp + " " + Op + " " +
                                       usrId + " " + tvouDt); */

                    /** call database function "INS_TEMP_TDS_LINE_EXP" to generate tds rule lines according to rule selected in tds rule and amount specified. */
                    callStoredProcedure("FIN.INS_TEMP_TDS_LINE_EXP(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                        new Object[] { cldId, slocId, hoOrgId, orgId, instId, vouId, tvouDt, tvouSlNo,
                                                       billSlNo, ruleid, finAmt, currIdsp, usrId, tvouDt, Op });


                } else if (lineRow.getAttribute("TvouTdsFlg").equals("N") &&
                    ...
                    /* System.out.println("function called" + slocId + " " + orgId + " " + vouId + " " + tvouDt + " " +
                                       billSlNo + " " + ruleid + " " + finAmt + " " + currIdsp + " " + Op + " " +
                                       usrId + " " + tvouDt); */
                    /** call database function "INS_TEMP_TDS_LINE_EXP" to delte tds rule lines  */
                    callStoredProcedure("FIN.INS_TEMP_TDS_LINE_EXP(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                        new Object[] { cldId, slocId, hoOrgId, orgId, instId, vouId, tvouDt, tvouSlNo,
                                                       billSlNo, ruleid, finAmt, currIdsp, usrId, tvouDt, Op });
    ...
    public void tdsConsolidationExpense() {
        try {
            ...
            //  System.out.println("COA ID in EXpense function call is---->" + coaIdExp);
            /* callStoredFunction(NUMBER, "FIN.FN_TVOU_TDS_EXP(?,?,?,?,?,?,?,?)",
                               new Object[] { vouId, vouTyp, vouSubTyp, vouInsId, tvouSpId, hoOrgId, cldId,
                                              coaIdExp }); */
            //move outside package and re organize args
        } catch (Exception e) {
        ...
        while (rit.hasNext()) {
            ...
            if (lineRow.getAttribute("TvouTaxFlg") != null) {
                ...
                if (lineRow.getAttribute("TvouTaxFlg").equals("Y") && lineRow.getAttribute("TvouCoaId") != null) {
                    ...
                    Op = "I";
                   /*  System.out.println(slocId + " " + orgId + " " + vouId + " " + tvouDt + " " + billSlNo + " " +
                                       taxRuleId + " " + finAmt + " " + currIdsp + " " + Op + " " + usrId + " " +
                                       tvouDt); */

                    /** call database function "INS_TEMP_TAX_LINE_EXP" to generate tds rule lines according to rule selected in tds rule and amount specified. */
                    callStoredProcedure("FIN.INS_TEMP_TAX_LINE_EXP(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                        new Object[] { cldId, slocId, hoOrgId, orgId, instId, vouId, tvouDt, tvouSlNo,
                                                       billSlNo, taxRuleId, finAmt, currIdsp, usrId, tvouDt, Op });
                    ...
                    /** call database function "INS_TEMP_TAX_LINE_EXP" to delet tds rule lines. */
                    callStoredProcedure("FIN.INS_TEMP_TAX_LINE_EXP(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                                        new Object[] { cldId, slocId, hoOrgId, orgId, instId, vouId, tvouDt, tvouSlNo,
                                                       billSlNo, taxRuleId, finAmt, currIdsp, usrId, tvouDt, Op });
    ...
    public void taxConsolidationExpense() {
        try {
            ...
            // Integer vouInsId = Integer.parseInt(tvouVRow.getAttribute("TvouApplInstId").toString());
            /*  callStoredFunction(NUMBER, "fin.PKG_FIN_TVOU.FN_TVOU_TAX(?,?,?,?,?,?,?)",
                               new Object[] { vouId, vouTyp, vouSubTyp, vouInsId, tvouSpId, hoOrgId, cldId }); */
            //move outside package and organize args
    ...
    public void voucherDeleteWdtId(String cld_id, Integer sloc_id, String ho_org, String org_id, Integer usr_id,
        ...
        try {
            callStoredFunction(NUMBER, "FIN.FN_TVOU_DOC_MISMAT_DEL(?,?,?,?)",
                               new Object[] { ho_org, org_id, cld_id, sloc_id });
    ...
    public void adjustTvouAdjAllRows(String p_action_typ) {
        ...
        while (rit.hasNext()) {
            Row row = rit.next();
            if (p_action_typ.compareToIgnoreCase("true") == 0) {
                ...
                if (((Number)row.getAttribute("TvouCurrCcTrx")).compareTo(0) == 0 ||
                    ...
                    System.out.println("check box set true");
                    //row.setAttribute("TvouRadjAmtAdj", row.getAttribute("TvouAdjAmtOT"));
                    //    System.out.println("Tvou AdjAmtOt---->"+row.getAttribute("TvouAdjAmtOT")+"TvouCurrCcTrx--->"+row.getAttribute("TvouCurrCcTrx"));
                    Number amt =
                        (((Number)row.getAttribute("TvouAdjAmtOT")).multiply((Number)row.getAttribute("TvouCcCurr"))).divide((Number)row.getAttribute("TvouCurrCcTrx"));
                    // row.setAttribute("TvouAmtTrx", amt.round(amt_digit));
                }
            } else {
                /** else deselect check box of every row and clear adjusted amount of row. */
                row.setAttribute("adjustAllTrans", false);
                /*  if (((Number)(row.getAttribute("TvouAdjAmtOT"))).compareTo((Number)(row.getAttribute("TvouRadjAmtAdj"))) ==
                       0) { */
                // row.setAttribute("TvouAmtTrx", new Number(0));
                // row.setAttribute("TvouRadjAmtAdj", new Number(0));
            }
    ...
    public Integer on_load_page(String cld_id, Integer sloc_id, String org_id, Integer usr_id) {
        ...
        callStoredFunctionParam(VARCHAR, "App.fn_get_usr_doc_access_param(?,?,?,?,?,?,?,?,?)",
                                new Object[] { cld_id, sloc_id, org_id, 55, usr_id });
    ...
    public void setARAPParameters(String p_cld_id, Integer p_slc_id, String p_ho_id, String p_org_id, Integer p_coa_id,
       ...
       /*  System.out.println(" setArApParameters " + "p_cld_id" + p_cld_id + " p_slc_id " + p_slc_id + " p_ho_id " +
                           p_ho_id + " p_org_id " + p_org_id + " p_coa_id " + p_coa_id + " p_eo_mst_id " +
                           p_eo_mst_id + " p_arap_flg " + p_arap_flg + " p_amt_typ " + p_amt_typ + " p_filter_Val " +
                           p_filter_Val + " p_bill_Frm_Dt " + p_bill_Frm_Dt + " p_bill_To_Dt " + p_bill_To_Dt +
                           " p_bill_Days " + p_bill_Days); */
     ...
     */
    public ViewObjectImpl getTvouAdjAutoVO() {
        return (ViewObjectImpl)findViewObject("TvouAdjAutoVO");
    }
    ...
    protected String fetchTvouAdjAuto(String p_cld_id, Integer p_slc_id, String p_ho_id, String p_org_id,
        ...
        while (rit.hasNext()) {
            ...
            BigDecimal convRate =
                (BigDecimal)callStoredFunction(Types.NUMERIC, "FIN.FN_TVOU_CROSSCONVRATE(?,?,?,?,?)", new Object[] { p_cld_id,
                                                                                                                     p_ho_id,
                                                                                                                     (Integer)tvouAdjAutoRow.getAttribute("TvouCurrIdTrx"),
                                                                                                                     (Integer)tvouAdjAutoRow.getAttribute("TvouAdjCurrIdSp"),
                                                                                                                     tvouTyp });
    ...
    public String insertTvouAdjDtlData(Object p_adv_amt, Row tvou_adj, Row tvou_adv_dtl) {
        ...
        tvouAdjDtlAutoRow.setAttribute("TvouAdjAmtAdj", p_adv_amt);
        ...
        tvouAdjDtlAutoRow.setAttribute("TvouRadjAmtSp", tvou_adv_dtl.getAttribute("TvouAdvAmtSp"));
        tvouAdjDtlAutoRow.setAttribute("TvouRadjAmtAdj", p_adv_amt);
        ...
        tvouAdjDtlAutoRow.setAttribute("TvouAdjDtlCurrIdAdj", tvou_adj.getAttribute("TvouAdjCurrIdAdj"));

        Integer tvouTyp = Integer.parseInt(getTvou1().getCurrentRow().getAttribute("TvouTypeId").toString());

        BigDecimal convRate =
            (BigDecimal)callStoredFunction(Types.NUMERIC, "FIN.FN_TVOU_CROSSCONVRATE(?,?,?,?,?)", new Object[] { tvouAdjDtlAutoRow.getAttribute("TvouCldId").toString(),
                                                                                                                 tvouAdjDtlAutoRow.getAttribute("TvouHoOrgId").toString(),
                                                                                                                 (Integer)tvouAdjDtlAutoRow.getAttribute("TvouCurrIdTrx"),
                                                                                                                 (Integer)tvouAdjDtlAutoRow.getAttribute("TvouAdjCurrIdSp"),
                                                                                                                 tvouTyp });
        Number rate;
        try {
            rate = new Number(convRate);
            ...
            while (adjAutoRit.hasNext()) {
                ...
                if (tvouAdjAutoRow.getAttribute("TvouAdjChkFlg") != null) {
                    if (tvouAdjAutoRow.getAttribute("TvouAdjChkFlg").equals("Y")) {
                        ...
                        Number remBal = ADFUtil.roundOffAmt((Number)tvouAdjAutoRow.getAttribute("RemainingBalanceTrans"));
                        tvouAdjAutoRow.setAttribute("TvouAmtTrx", remBal);
            ...
            while ((tvouAdvDtlAutoRit.hasNext() || tvouAdjAutoRit.hasNext())) {
                ...
                if (((Number)tvouAdvDtlAutoRow.getAttribute("TvouAdjAmtOT")).compareTo(0) == 1 &&
                    ...
                    if ("Y".equals(tvouAdjAutoRow.getAttribute("TvouAdjChkFlg")) &&
                        ...
                        amtDiff = ADFUtil.roundOffAmt(tvouAdvRemainOT.subtract(tvouAdjRemainOT));
                        ...
                        if (amtDiff.compareTo(0) == 1) {
                            ...
                            tvouAdvDtlAutoRow.setAttribute("TvouRemainOt", amtDiff);
                            ...
                            tvouAdvRemainOT = new Number(amtDiff);

                            insertTvouAdjDtlData(tvouAdjRemainOT, tvouAdjAutoRow, tvouAdvDtlAutoRow);
                        ...
                        //remaining advance is less than remaining bill amount
                            ...
                            //update tvouAdjAutoRow
                            tvouAdjAutoRow.setAttribute("TvouRemainOt", amtDiff.abs());
                            // System.out.println("TvouRemainOt " + tvouAdjAutoRow.getAttribute("TvouRemainOt"));
                            tvouAdjAutoRow.setAttribute("TvouRadjAmtAdj", tvouRadjAmtAdj.add(tvouAdvRemainOT));
                            ...
                            //update tvouAdvDtlAutoRow
                            tvouAdvDtlAutoRow.setAttribute("TvouRemainOt", new Number(0));
                            tvouAdvDtlAutoRow.setAttribute("TvouAmtTrx", tvouTrxAmt.add(tvouAdvRemainOT));
                            tvouAdvDtlAutoRow.setAttribute("TvouAdvAmtAdj", tvouTrxAmt.add(tvouAdvRemainOT));

                            insertTvouAdjDtlData(tvouAdvRemainOT, tvouAdjAutoRow, tvouAdvDtlAutoRow);
                            tvouAdvRemainOT = new Number(0);
    ...
    public String callWfFunctions(String cldId, Integer slcId, String orgId, Integer usrId, String tvouId,
        ...
        try {
            ...
            callStoredFunction(NUMBER, "APP.WF_INS_TXN(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                               new Object[] { slcId, cldId, orgId, docId, docTypId, wfId, tvouId, usrId, usrId, level,
    ...
    public Integer assignBnkInstrmtAmt(Number bnkInstrmntAmt, Number oldCurrRate, Number newCurrRate) {
        ...
        Number assignableAmt = bnkInstrmntAmt;
        Number amtDiff = new Number(0);
        ...
        if (bnkInstrmntAmt != null && bnkInstrmntAmt.compareTo(0) == 1)
            while (tvouAdjAutoRit.hasNext()) {
                ...
                if ("Y".equals(tvouAdjAutoRow.getTvouAdjChkFlg()) &&
                    ...
                    amtDiff = ADFUtil.roundOffAmt(assignableAmt.subtract(tvouAdjAutoRow.getTvouAdjAmtOT()));
                    ...
                    if (amtDiff.compareTo(0) == 1) {
                        ...
                        //update tvouAdjAutoRows
                        tvouAdjAutoRow.setTvouAmtTrx(tvouAdjAutoRow.getTvouAdjAmtOT());
                    ...
                    //remaining advance is less than remaining bill amount
                        ...
                        tvouAdjAutoRow.setTvouAdjAmtAdjThs(assignableAmt);
    ...
    public String CheckDuplicateTemplateName(String CldId, Integer SlocId, String HoOrgId, String TemplateName,
        ...
        Object callStoredFunction =
            callStoredFunction(Types.VARCHAR, "FN_CHK_TMPL_NAME_UNQE(?,?,?,?,?,?)", new Object[] { CldId, SlocId,
                                                                                                   HoOrgId,
                                                                                                   TemplateName +
                                                                                                   " -(U)", UsrId,
                                                                                                   "I" });
       ...
       // Number TvouCc = (Number)curRowTvou.getAttribute("TvouCc");
        ...
        for (int i = 0; i < filteredRows.length; i++) {
            ...
            if (count == 0) {
                ...
                //  tvouAdjDtlAutoRow.setAttribute("TvouRadjAmtAdj", currentRow.getAttribute("ArapAmtAdj"));


                //   tvouAdjDtlAutoRow.setAttribute("TvouAdjTab", curRowAdjAuto.getAttribute("TvouRadjTab"));
                //    tvouAdjDtlAutoRow.setAttribute("TvouAdjVouId", curRowAdjAuto.getAttribute("TvouRadjVouId"));
                //  tvouAdjDtlAutoRow.setAttribute("TvouAdjVouDt", curRowAdjAuto.getAttribute("TvouRadjVouDt"));
                //  tvouAdjDtlAutoRow.setAttribute("TvouAdjSlNo", curRowAdjAuto.getAttribute("TvouRadjSlNo"));
                //  tvouAdjDtlAutoRow.setAttribute("TvouAdjArapSlNo", curRowAdjAuto.getAttribute("TvouRadjArapSlNo"));
                ...
                //tvouAdjDtlAutoRow.setAttribute("TvouCurrIdTrx", TvouCurrIdSp);
                //     tvouAdjDtlAutoRow.setAttribute("TvouCurrCcTrx", TvouCc);

                //  tvouAdjDtlAutoRow.setAttribute("TvouAdjAmtAdj", new Number(0));
                //   tvouAdjDtlAutoRow.setAttribute("UsrIdCreate", tvouLinesRow.getAttribute("UsrIdCreate"));
                //default selection of check box for all generated rows
                ...
                //    tvouAdjDtlAutoRow.setAttribute("TvouRadjVouId", currentRow.getAttribute("ArapFlg"));
                ...
                BigDecimal convRate =
                    (BigDecimal)callStoredFunction(Types.NUMERIC, "FIN.FN_TVOU_CROSSCONVRATE(?,?,?,?,?)",
                                                   new Object[] { p_cld_id, p_ho_id,
                                                                  (Integer)tvouAdjDtlAutoRow.getAttribute("TvouCurrIdTrx"),
                                                                  (Integer)tvouAdjDtlAutoRow.getAttribute("TvouAdjCurrIdSp"),
                                                                  tvouTyp });

                /* System.out.println("tvouAdjDtlAutoRow.getAttribute(\"TvouCurrIdTrx\") = " +
                                   tvouAdjDtlAutoRow.getAttribute("TvouCurrIdTrx") +
                                   "(Integer)tvouAdjDtlAutoRow.getAttribute(\"TvouAdjCurrIdSp\") = " +
                                   (Integer)tvouAdjDtlAutoRow.getAttribute("TvouAdjCurrIdSp") + "convRate = " +
                                   convRate); */
        ...
        // Code to Delete the inserted row from tvou_adj_Dtl when it is uncheck from arap popup @Amandeep
        /*  Row[] filteredRows1 = arapOsVo.getFilteredRows("InsertIntoAdjDtlFlag", "N");
        for(int j=0;j<filteredRows1.length;j++){
            String DocTxnIdDisp1 = (String)filteredRows1[j].getAttribute("DocTxnIdDisp");
            if(m.containsKey(DocTxnIdDisp1)){
                ViewObjectImpl adjDtlAutoVO = getTvouAdjDtlAutoVO();
                Row[] row = adjDtlAutoVO.getFilteredRows("TvouRadjDispId", DocTxnIdDisp1);
                if(row.length>0){
                    row[0].remove();
                }
            }
    ...
    public void selectAllAdvanceInAdjDtl() {
       ...
       // System.out.println("TvouAdjAmtOT = " + TvouAdjAmtOT);
        TvouAdjAmtOT = ADFUtil.roundOffAmt(TvouAdjAmtOT.multiply(TvouCcCurr).divide(TvouCurrCcTrx));
        ...
        while (rsi.hasNext()) {
            ...
            if (total.compareTo(TvouAdjAmtOT) <= 0) {
                ...
                Number TvouCcCurrDtl = (Number)next.getAttribute("TvouCcCurr");

         //       System.out.println("TvouRemainOT = " + TvouRemainOT); // NEED TO  CONVERT INTO TRXCC
                //   Number TvouAdjAmtAdj = (Number)next.getAttribute("TvouAdjAmtAdj");
                TvouRemainOT = ADFUtil.roundOffAmt(TvouRemainOT.multiply(TvouCcCurrDtl).divide(TvouCurrCcTrxDtl));
           //     System.out.println("TvouRemainOT after converasion= " + TvouRemainOT);
                if (TvouRemainOT.subtract(TvouAdjAmtOT.subtract(total)).compareTo(0) > 0) {
             //       System.out.println("total in if= " + total + "TvouAdjAmtOT.subtract(total) = " +
               //                        TvouAdjAmtOT.subtract(total));
                    next.setAttribute("TvouAmtTrx", TvouAdjAmtOT.subtract(total));
                    // next.setAttribute("TvouRadjAmtAdj", TvouAdjAmtOT.subtract(total));

                    total = total.add(TvouRemainOT);
                    //   next.setAttribute("TvouRemainOT", TvouRemainOT.subtract(TvouAdjAmtOT.subtract(total)));
                } else {
                    total = total.add(TvouRemainOT);
                 //   System.out.println("total in else = " + total + "TvouRemainOT =" + TvouRemainOT);
                    next.setAttribute("TvouAmtTrx", TvouRemainOT);
                    //next.setAttribute("TvouRadjAmtAdj", TvouRemainOT);
    ...
    public void setAutoAdjustGlobalParameters(String p_cld_id, Integer p_slc_id, String p_ho_id, String p_org_id,
        ...
        if (ADFUtil.resolvEl("#{pageFlowScope.PARAM_MODE}").toString().equalsIgnoreCase("E") || ADFUtil.resolvEl("#{pageFlowScope.PARAM_MODE}").toString().equalsIgnoreCase("V")||ADFUtil.resolvEl("#{pageFlowScope.PARAM_MODE}").toString().equalsIgnoreCase("S")) {
    ...
    public Number adjustAmount() {
        ...
        if (adjDtlSumCol.compareTo(-1) > 0) {
            ...
            currentRow.setAttribute("TvouRadjAmtAdj",
                                    ADFUtil.roundOffAmt(adjDtlSumCol.multiply(TvouCurrCcTrx).divide(TvouCcCurr)));
    ...
    public Number setTvouAdjDtlSumCol() {
        ...
        if (currentRow != null) {
            ...
            if (currentRow.getAttribute("TvouAmtTrx") != null) {
                ...
                trxAmtAdj = ADFUtil.roundOffAmt(trxAmtAdj.multiply(TvouCurrCcTrx).divide(TvouCcCurr));
           ...
           // System.out.println("total transaction amount according to bank currency = " + SumAmt);
            ...
            SumAmt = ADFUtil.roundOffAmt(SumAmt.multiply(TvouCurrCcTrx).divide(TvouCcCurr));
           // System.out.println("total transaction amount according to adj currency = " + SumAmt);
            ...
            if (remBal.compareTo(SumAmt) < 0) {
             //   System.out.println("return -1");
                ...
                /// ViewObject tvouAdjDtlVO = findViewObject("TvouAdjDtlAutoVO");
                /* ViewObjectImpl childDtl = getTvouAdjDtlAutoVO();
                RowSetIterator childDtlRit = childDtl.createRowSetIterator(null); */
                ...
                while (childDtlRit.hasNext()) {
                    ...
                    Number TvouCcCurrDtl = (Number)childRow.getAttribute("TvouCcCurr");
               ...
               //     System.out.println("Display Id=" + dispId);
                    ...
                    if (childRow.getAttribute("TvouAmtTrx") != null) {
                        ...
                        amount = ADFUtil.roundOffAmt(trxAmt.multiply(TvouCurrCcTrxDtl).divide(TvouCcCurrDtl));

                        // Changed this value to set in Bill currency
                        childRow.setAttribute("TvouAdjAmtAdj",
                                              ADFUtil.roundOffAmt(trxAmt.multiply(TvouCurrCcTrx).divide(TvouCcCurr))); // need to convert into adj_dtl curr
                        childRow.setAttribute("TvouRadjAmtAdj", amount); // need to convert into adj_dtl curr
                        //tvouAdjRow.setAttribute("TvouRadjAmtAdj", tvouAdjRow.getAttribute("TvouAmtTrx"));TvouRadjDispId
                    ...
                    if (childRow.getAttribute("TotalAdjustedAmount") != null) {
                       /*  System.out.println("totalAdj = " + totalAdj + "amount = " + amount + " prevAdjustedAmount = " +
                                           prevAdjustedAmount + " amount.subtract(prevAdjustedAmount) = " +
                                           amount.subtract(prevAdjustedAmount) + ""); */
                        totalAdj = totalAdj.add(amount.subtract(prevAdjustedAmount));
                        // totalAdj = totalAdj.add((Number)childRow.getAttribute("TotalAdjustedAmount"));
                        childRow.setAttribute("TotalAdjustedAmount", totalAdj);
                       // System.out.println("new total adjustment amount=" + totalAdj + " amount = " + amount);
                        Row[] filteredRows = adjDtl.getFilteredRows("TvouRadjDispId", dispId);
                        //System.out.println("filteredRows = " + filteredRows.length);
                        for (int i = 0; i < filteredRows.length; i++) {
                            filteredRows[i].setAttribute("TotalAdjustedAmount", totalAdj);

                          //  System.out.println("data after setting = " + childRow.getAttribute("TotalAdjustedAmount"));
                        }
    ...
    public void deleteFromDetail(String displayId) {
        ...
        //System.out.println("adjDtl.length = " + adjDtl.getRowCount());
        //System.out.println("adjDtl1.length = " + getTvouAdjDtlAutoVO1().getRowCount());
    }
    ...
    public String checkBillNumberUniqueness(String CldId, Integer SlocId, String HoOrgId, String OrgId, Integer CoaId,
        ...
        if (CoaId != null) {
            Object callStoredFunction =
                callStoredFunction(Types.VARCHAR, "fin.fn_is_coa_dupli_bill_no(?,?,?,?,?,?,?,?,?)", new Object[] { CldId,
                                                                                                               SlocId,
                                                                                                               HoOrgId,
                                                                                                               OrgId,
                                                                                                               CoaId,
    ...
    public String checkExpenseBillNumberDuplicate(String CldId, Integer SlocId, String HoOrgId, String OrgId, Integer CoaId,
                                            Date vDate, String BillNo,String TvouId,String Mode){
        ...
        if(bill!=null){
            ...
            while(rsi.hasNext()){
                ...
                String billUnique = checkBillNumberUniqueness(CldId, SlocId, HoOrgId, OrgId, CoaId, vDate, BillNo,TvouId,Mode);
    ...
    public String checkInstrumentAddedOrNot() {
        ...
        if (lines != null) {
         ...
         //   System.out.println("lines.getRowCount()" + lines.getRowCount());
            while (rsi.hasNext()) {
                ...
                if (CoaTypeTrans != null) {
                    if (CoaTypeTrans.equals(78) && (!TvouLnTyp.equals("H"))) {
           ...
           // System.out.println("beforwe returning N");
            return "N";
        }
                                ...
                                if (CoaTypeTrans != CoaTypeHd) {
                                    ...
                                    if (chq != null) {
                                        ...
                                        if (allRowsInRange.length>0) {
                                            ...
                                            System.out.println("chq.getFilteredRows for slNo = "+TvouSlNo +" is filteredRows = "+filteredRows.length);
                                            if (filteredRows.length > 0) {
                                                Object TvouAmtTyp = next.getAttribute("TvouAmtTyp");
                                                System.out.println(" in instruemnt TvouAmtTyp = "+TvouAmtTyp+" CoaTypeTrans = "+CoaTypeTrans);
                                                if ((CoaTypeTrans.equals(78) && TvouAmtTyp.equals("Dr")) || (CoaTypeTrans.equals(79) && TvouAmtTyp.equals("Cr"))) {
                                                    //only dtr 736
                                                    for (int i = 0; i < filteredRows.length; i++) {
                                                ...
                                                } else if ((CoaTypeTrans.equals(78) && TvouAmtTyp.equals("Cr")) || (CoaTypeTrans.equals(79) && TvouAmtTyp.equals("Dr"))) {
                                                    // 111,740, 112,238,239
