...
public class GateEntryAddEditBean implements Serializable{
    ...
    public void selectItemAction(ActionEvent actionEvent) {
        ...
        if(sourceDocType.getValue()!=null ){
          if(sourceDocType.getValue().toString().trim().length()>0){
            ...
            if((sourceDocType.getValue().toString().equals(SOURCE_DOC_TYPE_WOUT_PO.toString()) ) || (sourceDocType.getValue().toString().equals(SOURCE_DOC_TYPE_WOUT_SO.toString()))){
             ...
             String type=null;
                ...
                _log.info("Count=="+geTreeTable.getRowCount());
                    ...
                    _log.info("itm=="+itmNameBindVar.getValue());
                            if(itmNameBindVar.getValue()!=null && itmNameBindVar.getValue().toString().trim().length()>0){
                                if(uomBindVar.getValue()!=null){
                                    ...
                                    if (qty.compareTo(new Number(0))==1){
                                      ...
                                      if("N".equalsIgnoreCase(chkItmDup.getResult().toString())){      
                                        OperationBinding addItmOp = getBindings().getOperationBinding("addItemToGe");
                                        ...
                                        if(addItmOp.getErrors().isEmpty()){
                                            OperationBinding setnull = getBindings().getOperationBinding("setNullforAll");
                                            setnull.execute();
                                          ...
                                          String exit_msg = resolvElDCMsg("#{bundle['MSG.427']}").toString();      //Item already exists.
                                          ...
                                          showFacesMessage(exit_msg, "E", false, "F");
                                            ...
                                            showFacesMessage(resolvElDCMsg("#{bundle['MSG.337']}").toString(), "E", false, "F");
                    ...
                    if((sourceDocType.getValue().toString().equals(SOURCE_DOC_TYPE_WOUT_PO.toString()) ))
                    showFacesMessage(resolvElDCMsg("#{bundle['MSG.660']}").toString(), "E", false, "F");   //  showFacesMessage("Please Select Supplier.", "E", false, "F");
                 else
                   if(sourceDocType.getValue().toString().equals(SOURCE_DOC_TYPE_WOUT_SO.toString()))
            ...
            if(sourceDocNo.getValue()!=null && sourceDocNo.getValue().toString().trim().length()>0){
                ...
                if("Y".equals(dupChk.getResult().toString())){
                   ...
                   showFacesMessage("MSG.375","E",true,"F");
                    ...
                    OperationBinding chkCurr = getBindings().getOperationBinding("checkCurrCompatibility");
                    ...
                    if("Y".equalsIgnoreCase(chkCurr.getResult().toString())) { 
                        ...
                        _log.info("Warehouse OK");
                         ...
                         OperationBinding itmPop = getBindings().getOperationBinding("getItemsForPopUp");
                             itmPop.getParamsMap().put("docType",Integer.parseInt(sourceDocType.getValue().toString()));
                    ...
                    System.out.println("--"+sourceDocNo.getValue());
                                 ...
                                 if(sourceDocType!=null && sourceDocNo!=null){
                                     ...
                                     //    _log.info("--Populate from Po call--");
                                             ...
                                             if(popGeItmPo.getErrors().size()==0)
                                                ...
                                                System.out.println("Error while populateGeItmfromPo is == "+popGeItmPo.getErrors()); 
                                             if(noOfRows==0)
                                             {
                                             String item_msg=resolvElDCMsg("#{bundle['MSG.661']}").toString();
                                      FacesContext.getCurrentInstance().addMessage(null,new FacesMessage(FacesMessage.SEVERITY_INFO,item_msg,null));  //"This PO have no item for this Schedule No."  
                                                 }
                                             else
                                             {
                        ...
                        showFacesMessage(resolvElDCMsg("#{bundle['MSG.663']}").toString(), "E", false, "F");
                 ...
                 showFacesMessage(resolvElDCMsg("#{bundle['MSG.664']}").toString(), "E", false, "F");
                  ...
                  showFacesMessage(resolvElDCMsg("#{bundle['MSG.660']}").toString(), "E", false, "F"); //Please Select Supplier.
              }
                        ...
                        linkitm.execute(); 
                            OperationBinding chkItmExist=getBindings().getOperationBinding("chkItmTrfQty");
                                ...
                                if(chkItmExist.getErrors().size()==0 && chkItmExist.getResult()!=null)
                                {
                                    ...
                                    {
                                OperationBinding popGeItmPo = getBindings().getOperationBinding("populateGeItmfromTrfOrd");
                                popGeItmPo.execute();
                                OperationBinding exec = getBindings().getOperationBinding("Execute1");
                                exec.execute();
                                OperationBinding execPar = getBindings().getOperationBinding("Execute");
                ...
                }else if(Integer.parseInt(sourceDocType.getValue().toString())==SOURCE_DOC_TYPE_CPO)
            {
                ...
                if(sourceDocNo.getValue()!=null && sourceDocNo.getValue().toString().trim().length()>0){
                    ...
                    if("Y".equals(dupChk.getResult().toString())){
                       ...
                       showFacesMessage("MSG.375","E",true,"F");
                                     ...
                                     if(sourceDocType!=null && sourceDocNo!=null){
                                         if(Integer.parseInt(sourceDocType.getValue().toString())==SOURCE_DOC_TYPE_CPO){
                                         //    _log.info("--Populate from Po call--");
                                                 OperationBinding popGeItmCPo = getBindings().getOperationBinding("populateGeItmfromCPo");
                                                 popGeItmCPo.execute();
                                                 Integer noOfRows=0;
                                                 if(popGeItmCPo.getErrors().size()==0)
                                                    noOfRows = (Integer)popGeItmCPo.getResult();
                                                 else
                                                    System.out.println("Error while populateGeItmfromCPo is == "+popGeItmCPo.getErrors()); 
                                                 if(noOfRows==0)
                                                 {
              ...
              showFacesMessage(resolvElDCMsg("#{bundle['MSG.665']}").toString(), "E", false, "F");  //Please Select the Document Type
    ...
    public String editGeAction() {
        ...
        if(((Integer)geStatus.getValue())==298 ){
            ...
            FacesContext.getCurrentInstance().addMessage(null,new FacesMessage(FacesMessage.SEVERITY_INFO,resolvElDCMsg("#{bundle['MSG.666']}").toString(),null));  //The Materials are transfered to store. Cannot Edit.
            
        }else if(((Integer)geStatus.getValue())==453)
            {
                 FacesContext.getCurrentInstance().addMessage(null,new FacesMessage(FacesMessage.SEVERITY_INFO,"Material Received at Store, Cannot Edit.",null));  //Material Received at Store, Cannot Edit.
             }
            else
            {
    ...
    public String saveGeAction() {
       ...
       if("Y".equalsIgnoreCase(chkDocPr.getResult().toString())){
           ...
           if(dh!=null && !dh.isEmpty())
           {
                   ...
                   String msg ="<html><body><p>Some items do not have Delivery quantity for the following documents : <b>" + dh +
                                        "</b> </p></body></html>";  
     ...
     else 
     {
       ...
       if(h!=null && !h.isEmpty()){
           ...
           StringBuilder msg =
               new StringBuilder("<html><body><p>Some items do not have received quantity for the following documents : <b>" + h +
                                 "</b> </p>");    //MSG.705
          ...
          if("Y".equalsIgnoreCase(chkDocPr.getResult().toString())){
           ...
           else
           {
               ...
               /* if(Integer.parseInt(sourceDocType.getValue().toString())==SOURCE_DOC_TYPE_PO || Integer.parseInt(sourceDocType.getValue().toString())==SOURCE_DOC_TYPE_CPO){
                   ...
                   showFacesMessage(resolvElDCMsg("#{bundle['MSG.667']}").toString(), "E", false, "F");  //Documents have not been added for this Gate Entry.Cannot save.
               }
            }
       }
       }else{
           showFacesMessage(resolvElDCMsg("#{bundle['MSG.667']}").toString(), "E", false, "F");  //"Documents have not been added for this Gate Entry.Cannot save."
            ...
            FacesContext.getCurrentInstance().addMessage(null,new FacesMessage(FacesMessage.SEVERITY_ERROR,resolvElDCMsg("#{bundle['MSG.668']}").toString(),null));       //"Gate Entry Date is Required"            
      }
        return null;
    }
    ...
    public void rcptSrcTypeVCE(ValueChangeEvent valueChangeEvent) {
        if(valueChangeEvent.getNewValue()!=null){
           /*  Integer vc=(Integer)valueChangeEvent.getNewValue();
            ...
            if(vc==262|| vc==263){
            OperationBinding opC=getBindings().getOperationBinding("setDocumentTypeGe");
            opC.execute();
            } */
    ...
    public void rcptQtyValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        if(object!=null){
            ...
            if(((Number)object).compareTo(zero)==-1){
                showFacesMessage(resolvElDCMsg("#{bundle['MSG.767']}").toString(), "E", false, "V");   //"Received Quantity cannot be less than zero."
            }
            else
            {
    ...
    public void dlvQtyValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        if(object!=null){
          ...
          //  _log.info("Delivery Quantity in VAlidaor--"+(Number)object);
            if(((Number)object).compareTo(zero)==-1 ){
                showFacesMessage(resolvElDCMsg("#{bundle['MSG.767']}").toString(), "E", false, "V");   //Delivery Quantity cannot be less than zero.
            ...
            else
            {
                    Boolean is=    isPrecisionValid(26, 6, ((Number)object));
                              ...
                              //Compare to TotRcptQty
                               ...
                               {
                                       throw new ValidatorException(new
                                       FacesMessage(FacesMessage.SEVERITY_ERROR,"Return Quantity can not be Greater than Total Receipt Quantity.",null));
                                   }
                               }
                               else
                               {
