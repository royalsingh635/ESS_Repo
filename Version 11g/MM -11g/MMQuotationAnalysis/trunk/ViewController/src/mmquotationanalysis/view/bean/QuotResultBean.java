...
public class QuotResultBean {
    ...
    public String ViewQuotparamLink() {
        ...
        String rfqId = (String)vo.getCurrentRow().getAttribute("RfqDocId");
        String eval_id = (String)vo.getCurrentRow().getAttribute("EvalId");
    ...
    public void generatePODialogListener(DialogEvent dialogEvent) {
        ...
        if (dialogEvent.getOutcome().name().equals("yes")) {
            ...
            System.out.println("P_SLOCID "+P_SLOCID+"P_USERID "+P_USERID+"P_ORGID "+P_ORGID+"P_CldID "+P_CldID+"P_QUOTDOCID "+P_QUOTDOCID);
            System.out.println(" PO ID before function call "+poId);
            Timestamp dt = new Timestamp(System.currentTimeMillis());
                Integer fyid=(Integer)(callStoredFunction(Types.INTEGER, "APP.GET_ORG_FY_ID (?,?,?)", new Object[] {P_CldID,P_ORGID,dt}));
            poId = (String)callStoredFunction(Types.VARCHAR,"PKG_MM_QUOT.GEN_DRFT_PO_FRM_QUOT(?,?,?,?,?,?,?)", new Object[] { P_SLOCID, P_CldID , P_ORGID,P_QUOTDOCID,170, P_USERID,fyid });
           // quotSelectBind.setDisabled(true);
            ...
            if(poId != null){
                FacesMessage message = new FacesMessage(resolvElDCMsg("#{bundle['LBL.769']}").toString()+poId+" "+resolvElDCMsg("#{bundle['MSG.319']}").toString());
    ...
    public String saveAndForwardAction() {
      ...
      System.out.println("sloc_id "+sloc_id+"cld_id   "+cld_id+"pOrgId   "+pOrgId+"WfNum "+WfNum+"poDocId "+quotAnaEvalId+"usr_idFrm  "+usr_id+"usr_idTo   "+usr_id+"levelFrm "+level+"levelTo  "+level+"action  "+action+"remark  "+remark);
      OperationBinding insTxnOp = getBindings().getOperationBinding("insIntoTxn");
        ...
        {
                FacesMessage message2 = new FacesMessage("Something went wrong with user level in Workflow.Contact to ESS!");
                message2.setSeverity(FacesMessage.SEVERITY_ERROR);
                FacesContext fc = FacesContext.getCurrentInstance();
        ...
        {
                FacesMessage message2 = new FacesMessage("Workflow not Define for this Organisation.");
                message2.setSeverity(FacesMessage.SEVERITY_ERROR);
                FacesContext fc = FacesContext.getCurrentInstance();
                fc.addMessage(null, message2);
                return null;
    ...
    quotationAnaAMImpl am = (quotationAnaAMImpl)resolvElDC("quotationAnaAMDataControl");
            ...
            if(r.length>0)
                ...
                String msg2 = "This Quotation Analysis is pending at ["+name+"] for approval, You can not forward this.";
                    FacesMessage message2 = new FacesMessage(msg2);
                    message2.setSeverity(FacesMessage.SEVERITY_ERROR);
                    FacesContext fc = FacesContext.getCurrentInstance();
                    fc.addMessage(null, message2);
