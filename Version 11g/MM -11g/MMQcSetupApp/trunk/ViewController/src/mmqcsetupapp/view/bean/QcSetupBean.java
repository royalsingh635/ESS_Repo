...
public class QcSetupBean {
   ...
   public static ADFLogger  adfLog = ADFLogger.createADFLogger(QcSetupBean.class);
                         ...
                         if(isEditable.getResult()!=null){
                             ...
                             if("Y".equalsIgnoreCase(retVal)){
                                 ...
                                 FacesMessage msg =
                                   ...
                                   new FacesMessage(FacesMessage.SEVERITY_ERROR, msge, null);
                                 FacesContext fc = FacesContext.getCurrentInstance();
              ...
              if(isItmReq.getResult()!=null){
                  if("Y".equalsIgnoreCase(isItmReq.getResult().toString())){
                      FacesMessage msg =
                         ...
                         new FacesMessage(FacesMessage.SEVERITY_ERROR,"Item Name Required.",null);
                      FacesContext fc = FacesContext.getCurrentInstance();
        ...
        OperationBinding isItmReq = getBindings().getOperationBinding("isGrpRequired");
              ...
              if(isItmReq.getResult()!=null){
                  if("Y".equalsIgnoreCase(isItmReq.getResult().toString())){
                      FacesMessage msg =
                         // new FacesMessage(FacesMessage.SEVERITY_ERROR,"Paramater used in other application. You can't delete .!!",null);
                         new FacesMessage(FacesMessage.SEVERITY_ERROR,"Group Required.",null);
                      FacesContext fc = FacesContext.getCurrentInstance();
    ...
    public String saveGrpAction() {
        ...
        adfLog.info("grpId   "+grpId+"Mode fuc"+getModeForFunction()+"p_sloc_id   "+p_sloc_id+"p_org_id  "+p_org_id+"p_cldId    "+p_cldId);
    ...
    public void tlrncLowerValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
       ...
       Number stdVal =(Number)stdValNew.abs();
       if(stdVal == null){
           stdVal = zero;
       }
       adfLog.info("stdValNew >> "+stdValNew+"  stdVal >> "+stdVal);
       if(lwrVal !=null && tlrncTypeBind.getValue() != null && stdVal != null){
       if(lwrVal.compareTo(zero) < 0){
            //throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          //"Value must be greater than or equals zero", null));
                    throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                             resolvElDCMsg("#{bundle['MSG.653']}").toString(), null));
            }
            if(tlrncTypeBind.getValue().equals("A") && lwrVal.compareTo(stdVal)==1 && (zero.compareTo(stdVal)>0 || zero.compareTo(stdVal)<0 )){
                //throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Value Less than or equals to "+stdVal, null));
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.654']}").toString()+stdVal, null));
            }else if(tlrncTypeBind.getValue().equals("A") && lwrVal.compareTo(stdVal)==1 && zero.compareTo(stdVal)==0){
               // throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Value must be equals to "+stdVal, null));
               throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,resolvElDCMsg("#{bundle['MSG.655']}").toString() +stdVal, null));
            }else if(tlrncTypeBind.getValue().equals("P") && lwrVal.compareTo(hundrd) == 1){
            //throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Invalid Percentage",null));
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.303']}").toString(),null));
        }else if(tlrncTypeBind.getValue().equals("P") && lwrVal.compareTo(zero) == -1){
            //throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Invalid Percentage",null));
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.303']}").toString(),
                                                                      null));
            }else if(tlrncTypeBind.getValue().equals("P") && zero.compareTo(stdVal)==0 && lwrVal.compareTo(zero)==1){
                //throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Value must be equals to "+stdVal, null));
                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.655']}").toString()+stdVal, null));
            }
       }
    ...
    public void tlrncLwrGrpValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if(lwrVal !=null){
         ...
         if(tlrncTypeGrpBind.getValue().equals("A") && lwrVal.compareTo(stdVal)==1 && (zero.compareTo(stdVal)>0 || zero.compareTo(stdVal)<0 )){
             //throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Value Less than or equals to "+stdVal, null));
             throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.654']}").toString()+stdVal, null));
         }else          if(tlrncTypeGrpBind.getValue().equals("A") && lwrVal.compareTo(stdVal)==1 && zero.compareTo(stdVal)==0){
             //throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Value must be equals to "+stdVal, null));
             throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.655']}").toString()+stdVal, null));
         }else if(tlrncTypeGrpBind.getValue().equals("P") && lwrVal.compareTo(hundrd) == 1){
             //throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Invalid Percentage",null));
             throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.303']}").toString(),null));
                                                           
         }else if(tlrncTypeGrpBind.getValue().equals("P") && zero.compareTo(stdVal)==0 && lwrVal.compareTo(zero)==1){
               // throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Value must be equals to "+stdVal, null));
               throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, resolvElDCMsg("#{bundle['MSG.655']}").toString()+stdVal, null));
            }
        }
    ...
    public String deleteParamAction() {
        ...
        if(isdelete.getResult()!=null){
            ...
            if("Y".equalsIgnoreCase(retVal)){
                ...
                FacesMessage msg =
                   // new FacesMessage(FacesMessage.SEVERITY_ERROR,"Paramater used in other application. You can't delete .!!",null);
                   new FacesMessage(FacesMessage.SEVERITY_ERROR,resolvElDCMsg("#{bundle['MSG.656']}").toString(),null);
                FacesContext fc = FacesContext.getCurrentInstance();
