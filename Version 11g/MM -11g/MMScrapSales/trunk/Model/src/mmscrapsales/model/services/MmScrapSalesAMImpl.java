...
public class MmScrapSalesAMImpl extends ApplicationModuleImpl {
    ...
    // String txnId = (String)callStoredFunction(Types.VARCHAR, "FN_MM_GEN_ID(?,?,?,?)", new Object[]{SlocId,CldId,OrgId,tableName}); 
     ...
     fyId =(Integer)(callStoredFunction(Types.INTEGER, "APP.GET_ORG_FY_ID (?,?,?)", new Object[] {CldId,OrgId,getMmScrpSls().getCurrentRow().getAttribute("DocDt")}));
     }else{
        fyId =(Integer)(callStoredFunction(Types.INTEGER, "APP.GET_ORG_FY_ID (?,?,?)", new Object[] {CldId,OrgId,getMmScrpSls().getCurrentRow().getAttribute("DocDt")}));
     }
    ...
    public String inserSrStock(){
        ...
        if ("Y".equalsIgnoreCase(srFlg)) {
                       ...
                       if (scrpsr.length == 0) {
                           ...
                           while (rwitr.hasNext()) {
                               Row r2 = rwitr.next();
                               if (r2.getAttribute("transcrpqty") != null && ((Boolean)r2.getAttribute("transcrpqty")) == true)  {
                                   RowQualifier rowQualifier1 = new RowQualifier(scrplotvo);
                                   rowQualifier1.setWhereClause("ItmId = '" + r1.getAttribute("ItmId") + "'and DocId= '"+r1.getAttribute("DocId")+"' AND LotId= '" +r2.getAttribute("LotId") + "' AND CldId='"+cldId+"' and OrgId ='"+orgId+"' and SlocId="+slocId+"");
                                   ...
                                   if(rows.length>0){
                                       rows[0].setAttribute("LotQtyBs", ((Number)rows[0].getAttribute("LotQtyBs")).add(new Number(1)));
                           ...
                           rwitr.closeRowSetIterator();
                               ...
                               while (rwitr.hasNext()) {
                                   ...
                                   if (((Boolean)r2.getAttribute("transcrpqty"))==false || ((Boolean)r2.getAttribute("transcrpqty"))==true) {
                                       RowQualifier rowQualifier = new RowQualifier(scrpsrvo);
                                       rowQualifier.setWhereClause("ItmId = '" + r1.getAttribute("ItmId") + "' AND LotId= '" +r2.getAttribute("LotId") + "'AND SrNo= '"+ r2.getAttribute("SrNo")+"'");
                                       ...
                                       if (rows.length > 0) {
                                           ...
                                           if (((Boolean)r2.getAttribute("transcrpqty"))==true) {
                                               ...
                                               if(rows1.length>0){
                                                  ...
                                                  rows1[0].setAttribute("LotQtyBs", ((Number)rows1[0].getAttribute("LotQtyBs")).add(new Number(1)));
                                               }
                                               else if(rows.length==0){
                                                        adflog.info("lot is create first"+r2.getAttribute("LotId"));
                                                        Row scrplot = scrplotvo.createRow();
   ...
   System.out.println("DocId:"+docid);
              ...
              RowQualifier rowQualifier1 = new RowQualifier(this.getMmScrpSlsSr());
                ...
                if(filteredRows.length>0){
                    for(Row r1:filteredRows) {
                        ...
                        rowQualifier.setWhereClause("LotId='" + LotId + "' AND ItmId= '" + ItmId + "' AND SrNo ='"+ srno +"'");
    ...
    public String checkSaveRecord(){
       ...
       if(scrCurr!=null){
           ...
           if(ar.isEmpty() && ar2.isEmpty()){
               ...
               flag="N";
               FacesMessage message = new FacesMessage("These Item has improper stock"+ar.toString());
               ...
               flag="N";
               FacesMessage message = new FacesMessage("Some Item has Scrap Price zero"+ar2.toString());
               message.setSeverity(FacesMessage.SEVERITY_ERROR);
               FacesContext fc = FacesContext.getCurrentInstance();
               fc.addMessage(null, message);
    ...
    public String genscrapId(){
        ...
        if(curr!=null){
            ...
            if(curr.getAttribute("ScrpSlsId")!=null){
                    ...
                    System.out.println(curr.getAttribute("SlocId")+" "+curr.getAttribute("CldId")+" "+curr.getAttribute("OrgId"));
                    String scrapNo =(String)(callStoredFunction(Types.VARCHAR, "FN_MM_GEN_ID (?,?,?,?,?,?)", new Object[] {slocId,cldId,orgId,"MM$SCRP$SLS",whId,fyId}));  
                    System.out.println(curr.getAttribute("SlocId")+" "+curr.getAttribute("CldId")+" "+curr.getAttribute("OrgId"));
                    if(scrapNo!=null){
    ...
    public String DeleteSrno(){
        ...
        if(curr!=null){
            RowQualifier rowQualifier1 = new RowQualifier(scrplotvo);
            rowQualifier1.setWhereClause("ItmId = '" + curr.getAttribute("ItmId") + "'AND DocId='"+curr.getAttribute("DocId")+"' AND LotId= '" +curr.getAttribute("LotId") + "' AND CldId='"+cldId+"' and OrgId ='"+orgId+"' and SlocId="+slocId+"");
              Row[] rows1 = scrplotvo.getFilteredRows(rowQualifier1);
            adflog.info("filtred lot value is "+rows1.length);
            ...
            if(rows1.length>0){
                if(((Number)rows1[0].getAttribute("LotQtyBs")).compareTo(new Number(0))==0){
                    ...
                    //for(Row r1:rows1){
                        rows1[0].setAttribute("LotQtyBs", ((Number)rows1[0].getAttribute("LotQtyBs")).subtract(new Number(1)));    
                   // }
    ...
    public String selectAllItem(){
        ...
        BigDecimal ItmIns =(BigDecimal)(callStoredFunction(Types.NUMERIC, "MM_INS_SCRP_SLS_ITM (?,?,?,?,?,?,?,?)", new Object[] {cldId,slocId,orgId,whId,"",DocId,FyId,"Y"}));  
    ...
    public String autoStkAllotment(){
        ...
        BigDecimal ItmIssue =(BigDecimal)(callStoredFunction(Types.NUMERIC, "MM_SCRP_SLS_AUTO_ISSU (?,?,?,?,?,?,?,?)", new Object[] {cldId,slocId,orgId,whId,ItmId,srFlg,DocId,scrpQty}));  
    ...
    public String itmValidChk(){
        ...
        rowQualifier1.setWhereClause("ItmId = '" + curr.getAttribute("TransItmId") + "'AND DocId='"+curr.getAttribute("DocId")+"' AND CldId='"+cldId+"' and OrgId ='"+orgId+"' and SlocId="+slocId+"");
    ...
    public String getWfNo(Integer SlocId,String CldId,String OrgId,Integer DocNo) 
       ...
       Integer DocNo1 = 18508; 
           ...
           return Integer.parseInt(callStoredFunction(NUMBER,"APP.WF_INS_TXN(?,?,?,?,?,?,?,?,?,?,?,?,?,?)",new Object[] { SlocId, CldId,OrgId, DocNo1,0, 
                                                                                                                          WfNo, DocId,usr_idFrm,
                                                                                                                          usr_idTo, levelFrm, levelTo, 
                                                                                                                          action, remark,amount}).toString());
       ...
       public String setOutCome(String whRtnvalue){
           ...
           System.out.println("CldId:"+cldId+" "+"SlocId"+slocId+"OrgId:"+orgId+"User Id:"+UsrId+"Whid:"+Whid+"FyId:"+fyid+" "+"DocId:"+docid);
            Object i= Integer.parseInt(callStoredFunction(NUMBER,"MM.MM_UPDT_STK_SCRP_SLS(?,?,?,?,?,?,?)", new Object[] {cldId,slocId,orgId,Whid,fyid,docid,UsrId}).toString());
            System.out.println("Function returned Value:"+i);
//---------------------------------------------------GL_LINES Function  Call---------------------------------------
            System.out.println("MM_INS_GL_SCRP_SLS"+slocId+" "+cldId+" "+hoOrgId+" "+orgId+" "+docid+" "+d+" "+UsrId);
            Object i1= Integer.parseInt(callStoredFunction(NUMBER,"MM.MM_INS_GL_SCRP_SLS(?,?,?,?,?,?,?)", new Object[] {slocId,cldId,hoOrgId,orgId,docid,d,UsrId}).toString());
            System.out.println("Returned From the Second Function:"+i1);
//----------------------------------------General Voucher Created Function Call-----------------------------------------
            String VouId= callStoredFunction(Types.VARCHAR,"MM.MM_GET_RCPT_PROV_GL (?,?,?,?,?,?)", new Object[] {cldId,slocId,orgId,docid,"S","NO"}).toString();
            System.out.println("Voucher ID:"+VouId);
            if(VouId!=null){
                   FacesMessage message = new FacesMessage("Saved and Generated Voucher Id:"+VouId);   
       ...
       public Integer pendingScrapSlsCheck(Integer SlocId,String CldId,String OrgId,Integer ScrpSlsDocNo){
           ...
           Object obj= callStoredFunction(NUMBER, "APP.WF_GET_CUR_USR(?,?,?,?,?,?)", new Object[] { SlocId,CldId,OrgId,ScrpSlsDocNo,docIdTxn,0});
           Integer usrId=null; 
           if(obj!=null){
               usrId=Integer.parseInt(obj.toString());
           }
