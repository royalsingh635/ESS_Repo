...
public class AppOpportunityAMImpl extends ApplicationModuleImpl implements AppOpportunityAM {
    ...
    public String getNewItmId(Integer p_SlocId, String p_CldId, String p_OrgId, String p_ItmDesc, String p_ItmPrefix,
        ...
        String exec2 =
            callStoredFunction(Types.VARCHAR, "app.FN_INS_ITM(?,?,?,?,?,?,?,?,)", new Object[] { p_SlocId, p_CldId,
                                                                                                 p_OrgId, p_ItmDesc,
                                                                                                 p_ItmPrefix, p_GrpId,
                                                                                                 p_Uom,
                                                                                                 p_UsrId }).toString();
...
(Integer)(callStoredFunction(Types.INTEGER, "APP.GET_ORG_FY_ID (?,?,?)", new Object[] { P_CldId, P_OrgId,
                                                                                        slsOpp.getCurrentRow().getAttribute("DocDt") }));
    ...
    public String getDispDocId() {
        ...
        if (slsOppRowCurrent != null) {
            adfLog.info("slsOppRowCurrent not null");
            P_SlocId = (Integer)slsOppRowCurrent.getAttribute("SlocId");
            adfLog.info(" P_SlocId: " + P_SlocId);
            P_FyId = Integer.parseInt(slsOppRowCurrent.getAttribute("FyId").toString());
            adfLog.info(" P_FyId: " + P_FyId);
            P_CldId = slsOppRowCurrent.getAttribute("CldId").toString();
            adfLog.info(" P_CldId: " + P_CldId);
            P_OrgId = slsOppRowCurrent.getAttribute("OrgId").toString();
            adfLog.info(" P_OrgId: " + P_OrgId);

            P_DocHexId = slsOppRowCurrent.getAttribute("DocId").toString();
            adfLog.info("P_DocId : " + P_DocId);
            adfLog.info(" P_DocHexId: " + P_DocHexId);


        } else if (slsOppRowCurrent.getAttribute("DocId") != null) {
            adfLog.info("slsOppRowCurrent DocId not null");
            P_SlocId = Integer.parseInt(resolvEl("#{pageFlowScope.GLBL_APP_SERV_LOC}").toString());
            adfLog.info(" P_SlocId: " + P_SlocId);

            P_CldId = (resolvEl("#{pageFlowScope.GLBL_APP_CLD_ID}").toString());
            ///adfLog.info(" P_CldId: " + P_CldId);
            P_OrgId = (resolvEl("#{pageFlowScope.GLBL_APP_USR }").toString());
            adfLog.info(" P_OrgId: " + P_OrgId);
            P_DocHexId = slsOppRowCurrent.getAttribute("DocId").toString();
            adfLog.info(" P_DocHexId: " + P_DocHexId);
        ...
        String exec2 =
            callStoredFunction(Types.VARCHAR, "SLS.GEN_DISP_DOC_ID(?,?,?,?,?,?,?,?)", new Object[] { P_SlocId, P_CldId,
                                                                                                     P_OrgId, P_DocId,
                                                                                                     P_DocHexId, null,
                                                                                                     P_TableName,
                                                                                                     P_FyId }).toString();

        adfLog.info(" exec2 disp doc id: " + exec2);
     ...
     * @return LOVCust
     */
    public ViewObjectImpl getLOVCust() {
        return (ViewObjectImpl)findViewObject("LOVCust");
    }
     ...
     * @return SearchColumn
     */
    public ViewObjectImpl getSearchColumn() {
        return (ViewObjectImpl)findViewObject("SearchColumn");
    }
    ...
    public String chkItm() {
        ...
        try {
            exec2 =
                    callStoredFunction(Types.VARCHAR, "MM.MM_IS_ITM_SERIALIZED(?,?,?,?)", new Object[] { CldId, SlocId, OrgId,
                                                                                                         ItemId }).toString();
    ...
    public Number getMaxItemPrice(String itmid) {
        ...
        BigDecimal price =
            (BigDecimal)callStoredFunction(Types.NUMERIC, "SLS.FN_GET_EO_LATEST_PRICE(?,?,?,?,?,?,?,?,?,?,?)",
                                           new Object[] { CldId, SlocId, OrgId, HoOrgId,
