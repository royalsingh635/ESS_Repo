...
public class SlsInvShipItmVORowImpl extends ViewRowImpl {
    ...
    public BigDecimal getItmAmtSp() {
        ...
        BigDecimal existingAmt = (getAttributeInternal(ITMAMTGSSP) == null ? new BigDecimal(0) : (BigDecimal)getAttributeInternal(ITMAMTGSSP));
        ...
        try {
            ...
            if (getTaxRuleFlg().equals("Y") || rulId != -1) {
                //System.out.println("tax Rule id :"+rulId);
                Object taxType =
        ...
        BigDecimal tempItmAmt = (getAttributeInternal(ITMAMTSP) == null ? new BigDecimal(0) :(BigDecimal) getAttributeInternal(ITMAMTSP));
        if ("Y".equals(typ.toString())) {
            //System.out.println("Tax Amount trans : "+gettaxAmtTrans());
            value = value.subtract(gettaxAmtTrans());
            if(value.compareTo(tempItmAmt) != 0){
                setItmAmtSp(value);
                setItmAmtBs(value.multiply(cc));  
                System.out.println("__ ItmAmtSp Inclusive updated to : "+value);
            }
        } else {
            if(value.compareTo(tempItmAmt) != 0){
                setItmAmtSp(value);
                setItmAmtBs(value.multiply(cc));   
                System.out.println("__ ItmAmtSp updated Exclusive to : "+value);
            }
        }
        //System.out.println("Itm Amt Sp : "+getAttributeInternal(ITMAMTSP));
    ...
    public BigDecimal gettaxAmtTrans() {
        ...
        if(invTr!=null){
            tx = (invTr.getAttribute("TaxAmtSp") == null ? new BigDecimal(0) : (BigDecimal)invTr.getAttribute("TaxAmtSp"));
        }
        return tx;
