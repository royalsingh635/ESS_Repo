...
public class SecurityBean {
    ...
    public String GoMaterialManagement() {
        ...
        try {
            ...
            if(driver.equalsIgnoreCase("MySQL-AB JDBC Driver")){
                profile_Freez =
                        (callStoredFunction(STRING, "call mm_chk_org_prf(?,?,?,?)", new Object[] { getCloud_id(), getSloc_id(),
                                                                                                 orgId,
                                                                                                 "FREEZ_PRF" }).toString());   
            }
            else
            {
            profile_Freez =
                    (callStoredFunction(STRING, "MM.mm_chk_org_prf(?,?,?,?)", new Object[] { getCloud_id(), getSloc_id(),
                                                                                             orgId,
                                                                                             "FREEZ_PRF" }).toString());
            }
            ...
            adflog.info("parameter values-->" + " sloc id :" + getSloc_id() + ": getCloud_id  : " + getCloud_id() +
                        " : orgId : " + orgId);
    ...
    public String goToSales() {
        ...
        try {
            ...
            if(driver.equalsIgnoreCase("MySQL-AB JDBC Driver")){
                profile_Freez =
                        (callProcedureOut(STRING, "call fn_sls_chk_org_prf(?,?,?,?,?)", new Object[] { getCloud_id(),
                                                                                                      getSloc_id(), orgId,
                                                                                                      "FREEZE_PRF" }).toString());   
            }
            else
            {
            profile_Freez =
                    (callStoredFunction(STRING, "SLS.fn_sls_chk_org_prf(?,?,?,?)", new Object[] { getCloud_id(),
                                                                                                  getSloc_id(), orgId,
                                                                                                  "FREEZE_PRF" }).toString());
            }
    ...
    public void getSalesProfileSetup() {
        ...
        try {
            ...
            if(driver.equalsIgnoreCase("MySQL-AB JDBC Driver")){
                wh_chk =
                        (callProcedureOut(STRING, "call fn_sls_chk_org_prf(?,?,?,?,?)", new Object[] { getCloud_id(), getSloc_id(),
                                                                                                      orgId,
                                                                                                      "USE_WH" }).toString());   
            }
            else
            {
            wh_chk =
                    (callStoredFunction(STRING, "SLS.fn_sls_chk_org_prf(?,?,?,?)", new Object[] { getCloud_id(), getSloc_id(),
                                                                                                  orgId,
                                                                                                  "USE_WH" }).toString());
            }
            //System.out.println("date format from funtion------>" + date_format);
