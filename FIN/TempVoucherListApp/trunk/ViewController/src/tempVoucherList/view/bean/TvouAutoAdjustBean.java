...
public class TvouAutoAdjustBean {
    ...
    public void InvoiceTrxAmtValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        //  trxAmt=roundOffAmt((trxAmt.divide((Number)tvouCcCurrBinding.getValue())).multiply((Number)tvouCurrCctrxBinding.getValue()));
        System.out.println("tvouCcCurrBinding.getValue() = "+tvouCcCurrBinding.getValue()+" tvouCurrCctrxBinding.getValue() = "+tvouCurrCctrxBinding.getValue());
        System.out.println("trxAmt = "+trxAmt);
        Number addTrx=roundOffAmt((add.multiply((Number)tvouCcCurrBinding.getValue())).divide((Number)tvouCurrCctrxBinding.getValue()));
        //System.out.println("outstandingAmt " + outstandingAmt);
        if (trxAmt.compareTo(addTrx) == 1) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvElDCMsg("#{bundle['MSG.489']}").toString() + " " + addTrx,
...
Number baseAmount=new Number(0);
            ...
            if(currentRow!=null){
                ...
                if(cc!=null){
                   ...
                   // ADFBeanUtils.callBindingsMethod("updateCostCenterAmt", null, null);
                    ADFBeanUtils.callBindingsMethod("updateCostCenterAmt", new Object[]{sumTrxAmtBind.getValue()}, new Object[]{"amountSp"});
    ...
    public void projectVCL(ValueChangeEvent valueChangeEvent) {
        ...
        if(valueChangeEvent.getNewValue()!=null){
            ADFBeanUtils.callBindingsMethod("setProject", new Object[]{valueChangeEvent.getNewValue()}, new Object[]{"mode"});
        }
    }
}
