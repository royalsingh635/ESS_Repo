...
public class TvouAutoAdjustBean {
    ...
    private RichOutputText tvouAdjCurrIdSpBinding;
    ...
    public void InvoiceTrxAmtValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (object == null) {
            String msg = (String) resolvEl("#{bundle['MSG.1976']}");  //Transaction Amount is Mandatory
            String msg1 = (String) resolvEl("#{bundle['MSG.1977']}");  //Please Enter Transaction Amount
        ...
        if (trxAmt.compareTo(0) == -1) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvElDCMsg("#{bundle['MSG.489']}").toString(),  //Invalid Amount
...
//     
        Object Adjsp=tvouAdjCurrIdSpBinding.getValue();
...
//                           (tvouCurrIdTrxBinding.getValue().equals(Adjsp)));
        ...
        if (trxAmt.compareTo(addTrx) == 1) {
            throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                          resolvElDCMsg("#{bundle['MSG.489']}").toString() + " " +  //Invalid Amount
                                                          addTrx,
                                                          resolvElDCMsg("#{bundle['MSG.429']}").toString() + " " +  //Can not adjust more than Outstanding amount which is
                                                          addTrx));
    ...
    public String fetchAdjDataAction() {
        ...
        if ("C".equalsIgnoreCase(resolveElExp("#{pageFlowScope.TVOU_LN_COA_BEHAV}").toString())) {
            ...
            if (frmDateBinding.getValue() == null) {
                String msg = (String) resolvEl("#{bundle['MSG.468']}");  //From Date is required.
                showFacesMsg(msg, null, FacesMessage.SEVERITY_ERROR, frmDateBinding);
                return null;
            }

            if (toDateBinding.getValue() == null) {
                String msg = (String) resolvEl("#{bundle['MSG.469']}");  //To Date is required.
...
//                                                    "amountSp" });
    ...
    public void setTvouAdjCurrIdSpBinding(RichOutputText tvouAdjCurrIdSpBinding) {
        this.tvouAdjCurrIdSpBinding = tvouAdjCurrIdSpBinding;
    }

    public RichOutputText getTvouAdjCurrIdSpBinding() {
