...
public class TaxRuleBean {
    ...
    private RichPopup taxCalcPopUpBind;
    private RichSelectBooleanCheckbox taxtdsBinding;
    private RichLink viewCalcBind;
    private RichPopup packagePopupBinding;
    private RichTable pckgTableBinding;
    ...
    public String createTaxRuleDetails() {
        ...
        if (isBasicCoaExist.equals("N"))
            isBasicCoaExist = (String) callStoredFunction(Types.VARCHAR, "fn_is_basic_coa_exist(?,?,?)", new Object[] {
        ...
        if (isBasicCoaExist.equals("Y")) {
            ...
            DCIteratorBinding dciter = (DCIteratorBinding) bindings.get("AppTaxRuleLineIterator");
            ...
            FacesMessage message =
                new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                 "Please First Create Basic Coa For the Current Head Office", null);
    ...
    public void okCancelDialogListener(DialogEvent dialogEvent) {
        ...
        if (dialogEvent.getOutcome().name().equals("ok")) {
            ...
            if (coaId.getValue() != null && coaId.getValue().toString().length() > 0) {
                ...
                callStoredProcedure("APP.APP_ARRANGE_SRNO_TAX_RULE_LINE(?,?,?,?)", new Object[] {
                                    P_TAX_RULE_ID, P_SLOC_ID, P_HO_ORG_ID, P_CLD_ID
                });
    ...
    public void okCancelDialogListenerRule(DialogEvent dialogEvent) {
        ...
        if (dialogEvent.getOutcome().name().equals("ok")) {
            ...
            /**  Check Finalize flag is Y or N?
             *    If 'N' --> Then No need to perform validations, Driectly Commit
             *    If 'Y' --> Then Validate Minimum number of Rows required is two
             *    If min 2 rows are not there --> Raise error message
             *    else if  one Basic Account is there or not if not there then --> Raise Error message
             *    else if priority are assigned properly or not . If not then --> Raise Error Message
             *    else Commit
             * **/
            if (finalise.equalsIgnoreCase("N")) {
                performCommitOperation();
                BindingContainer bindings = getBindings();
                OperationBinding operationBinding = bindings.getOperationBinding("insertDataIntoPckg");
                operationBinding.execute();
                ...
                if (vo != null) {
                    ...
                    if (count < 2) {
                        ...
                        if (countBasic == 1) {
                            Object object = finalise;
                            if (object != null) {
                                if (object.toString().equalsIgnoreCase("Y") ||
                                    object.toString().equalsIgnoreCase("true")) {
                                    BindingContainer bc = getBindings();
                                    OperationBinding binding = bc.getOperationBinding("finalizeValidation");
                                    binding.execute();
                                    Object res = binding.getResult();
                                    System.out.println("res = " + res);
                                    if (res != null) {
                                        if (res.toString().equalsIgnoreCase("N")) {
                                            //     throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,"Invalid Priority assigned in tax Rule Detail",null));
                                            FacesMessage message =
                                                new FacesMessage("Invalid Priority assigned in tax Rule Detail");
                                            message.setSeverity(FacesMessage.SEVERITY_ERROR);
                                            FacesContext fc = FacesContext.getCurrentInstance();
                                            fc.addMessage(null, message);
                                        } else {
                                        ...
                                        FacesMessage message =
                                            new FacesMessage("Error in Checking Finalize Validation");
                                        message.setSeverity(FacesMessage.SEVERITY_ERROR);
                                        FacesContext fc = FacesContext.getCurrentInstance();
                                        fc.addMessage(null, message);
                                    }
        ...
        AdfFacesContext.getCurrentInstance().addPartialTarget(viewCalcBind);
    ...
    public void viewTaxCalActionListener(ActionEvent actionEvent) {
        BindingContainer bc = getBindings();
        OperationBinding binding = bc.getOperationBinding("finalizeValidation");
        binding.execute();
        Object res = binding.getResult();
        ...
        if (res != null) {
            if (res.toString().equalsIgnoreCase("N")) {
                //     throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,"Invalid Priority assigned in tax Rule Detail",null));
                FacesMessage message = new FacesMessage("Invalid Priority assigned in tax Rule Detail");
                message.setSeverity(FacesMessage.SEVERITY_ERROR);
                FacesContext fc = FacesContext.getCurrentInstance();
                fc.addMessage(null, message);
            } else {
                showPopup(taxCalcPopUpBind, true);
                getAm().getAppTaxRuleLineVO().executeQuery();
                // getAm().getAppTaxRule().executeQuery();
    ...
    public void setTaxCalcPopUpBind(RichPopup taxCalcPopUpBind) {
        this.taxCalcPopUpBind = taxCalcPopUpBind;
    }

    public RichPopup getTaxCalcPopUpBind() {
        return taxCalcPopUpBind;
    }

    public void getTotalTaxCalculation() {
    ...
    public void finalizeValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        if (object != null && object.toString().equalsIgnoreCase("true")) {
            System.out.println("valuee of the object is==" + object);
            System.out.println("value of tax tds chseck box is===" + taxtdsBinding.getValue());
            BindingContainer bc = getBindings();
            if (taxtdsBinding.getValue().toString().equalsIgnoreCase("true")) {
                OperationBinding op = bc.getOperationBinding("getAppliedTaxAmount");
                op.execute();
                Number totaltax = (Number) op.getResult();
                System.out.println("amount return by the function is=" + totaltax);
                if (totaltax.compareTo(new Number(100)) > 0) {
                    throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR, null,
                                                                  "Applied Tax amount  must be less than Taxable amount while finalise the TDS Rule."));
                }
            }
        }
    ...
    public void setTaxtdsBinding(RichSelectBooleanCheckbox taxtdsBinding) {
        this.taxtdsBinding = taxtdsBinding;
    }

    public RichSelectBooleanCheckbox getTaxtdsBinding() {
        return taxtdsBinding;
    }

    public void setViewCalcBind(RichLink viewCalcBind) {
        this.viewCalcBind = viewCalcBind;
    }

    public RichLink getViewCalcBind() {
        return viewCalcBind;
    }
private String pckgMode="E";

    public void setPckgMode(String pckgMode) {
        this.pckgMode = pckgMode;
    }

    public String getPckgMode() {
        return pckgMode;
    }
    ...
    public String addPackage() {
        ...
        operationBinding.execute();
        pckgMode="A";
        showPopup(packagePopupBinding, true);
        return null;
    }
    ...
    public void setPackagePopupBinding(RichPopup packagePopupBinding) {
        this.packagePopupBinding = packagePopupBinding;
    }

    public RichPopup getPackagePopupBinding() {
        return packagePopupBinding;
    }

    public void packageDialogListner(DialogEvent dialogEvent) {
        // Add event code here...
        if(dialogEvent.getOutcome().name().toString().equalsIgnoreCase("ok")){
            BindingContainer bindings = getBindings();
            OperationBinding operationBinding = bindings.getOperationBinding("Commit");
            operationBinding.execute();
            AdfFacesContext.getCurrentInstance().addPartialTarget(pckgTableBinding);
        }
    }

    public void packageCancelListner(PopupCanceledEvent popupCanceledEvent) {
            BindingContainer bindings = getBindings();
    ...
    public void setPckgTableBinding(RichTable pckgTableBinding) {
        this.pckgTableBinding = pckgTableBinding;
    }

    public RichTable getPckgTableBinding() {
        return pckgTableBinding;
    }
}
