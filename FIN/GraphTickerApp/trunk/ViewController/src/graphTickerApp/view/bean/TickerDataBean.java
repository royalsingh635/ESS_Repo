...
public class TickerDataBean {
    ...
    private RichPanelFormLayout panelFormforalert;
    ...
    public TickerDataBean() {
        ...
        callStoredFunction(NUMBER, "FN_INS_TKR_DATA(?,?,?,?,?,?,?)", new Object[] {
                           CldId, HoOrgId, slocId, OrgId, pusrId, callTYpe, callVal
        });
        ...
        while (userAccessVO.hasNext()) {
            ...
            if (tkrPos == 1) {
                System.out.println("userAccessRow = " + userAccessRow.getFinTkrOpCurr() + "--" +
                                   userAccessRow.getFinTkrOpVal());
    ...
    public String createAction() {
        ...
        String activity_thread_Id = (String) callStoredFunction(VARCHAR, "app.pkg_app.get_activity_id(?,?)", new Object[] {
                                                                userId, docID
        });
    ...
    public void configure2link(ActionEvent actionEvent) {
        ...
        String s = "1";
        Integer i = 1;
        String abc = (String) callStoredFunction(VARCHAR, "PKG_FIN_GUI.FN_TKR_POPULATE_TO_TMP(?,?,?)", new Object[] {
                                                 s, i, tkrid2
        });
        showPopup(tickerPopup, true);
    }
    ...
    public void viewOptionLink(ActionEvent actionEvent) {
        ...
        v.executeQuery();
        v.setWhereClause(" FIN_TKR_USR_ID  =" + pusrId +
                         " and fin_tkr_id in (select fin_tkr_id from fin$tkr where FIN_TKR_ACTV = 'Y' and FIN_TKR_VISB = 'Y')");
        v.executeQuery();
    ...
    public void printDetail() {
        System.out.println("in print detail");
        UIOutput o1 =
            bean.getAlert("400", "275", EbizParams.GLBL_APP_CLD_ID(), EbizParams.GLBL_APP_SERV_LOC(),
                          EbizParams.GLBL_HO_ORG_ID(), EbizParams.GLBL_APP_USR_ORG(), EbizParams.GLBL_APP_USR(),
                          "00303");
        this.panelFormforalert.getChildren().add(o1);
        System.out.println("addind Refresh");
        AdfFacesContext.getCurrentInstance().addPartialTarget(this.panelFormforalert);
        AdfFacesContext.getCurrentInstance().addPartialTarget(o1);
        AdfFacesContext.getCurrentInstance().addPartialTarget(this.panelGroup);
    ...
    public void getUpdate(ClientEvent clientEvent) {
        ...
        bean.getUpdate(this.getPanelFormforalert(), EbizParams.GLBL_APP_CLD_ID(), EbizParams.GLBL_APP_SERV_LOC(),
                       EbizParams.GLBL_HO_ORG_ID(), EbizParams.GLBL_APP_USR_ORG(), EbizParams.GLBL_APP_USR(), "00303");
        AdfFacesContext.getCurrentInstance().addPartialTarget(this.panelFormforalert);
    ...
    public void setPanelFormforalert(RichPanelFormLayout panelFormforalert) {
        this.panelFormforalert = panelFormforalert;
    }

    public RichPanelFormLayout getPanelFormforalert() {
        return panelFormforalert;
    }
}
