...
public class NaturalAccount {
    ...
    public void SaveNa(ActionEvent actionEvent) {
        ...
        if(flag.equals("A")){
          ...
          //  System.out.println("AAYA KYA??");
            FacesContext fc = FacesContext.getCurrentInstance();
            FacesMessage message2 = new FacesMessage(resolvEl("#{bundle['MSG.2461']}")); //Budget calc logic is required  when Part of Budget is checked.
            ...
            operationBinding.execute();
            if(operationBinding.getErrors().isEmpty()){
                String msg ="#{bundle['MSG.91']}";  //Record Saved successfully
                ...
                fc.addMessage(this.budget_calc_logic_form_bind.getClientId(), message2);
                    ...
                    if(createOpBAddress.getErrors().isEmpty()){
                       ...
                       /*  OperationBinding createOpBAddress1 = bindings.getOperationBinding("Execute");
                         ...
                         */
                        String msg ="#{bundle['MSG.92']}";  //Record Updated Successfully.
                        FacesMessage message = new FacesMessage(resolvEl(msg)); 
                        message.setSeverity(FacesMessage.SEVERITY_INFO);
                        FacesContext fc = FacesContext.getCurrentInstance();
                        fc.addMessage(null, message);
    ...
    public void multiRefChangeListner(ValueChangeEvent vce) {
        ...
        if (flag.equalsIgnoreCase("E")) {
            ...
            if (oldVal == "true" && newVal == "false") {
                ...
                if (count > 1) {
                    String error = resolvEl("#{bundle['MSG.167']}");  //Multiple references found.
    ...
    public void deleteNaButton(ActionEvent actionEvent) {
        ...
        if (flagDel.equalsIgnoreCase("Y")) {
            FacesMessage msg =
                new FacesMessage(resolvEl("#{bundle['MSG.168']}"));  //Natural Account can't be deleted, as COA is created for this Natural Account
            msg.setSeverity(FacesMessage.SEVERITY_WARN);
            FacesContext.getCurrentInstance().addMessage(null, msg);
        } else if (typId == 5022) {
            FacesMessage msg = new FacesMessage(resolvEl("#{bundle['MSG.169']}"));  //Natural Account for tax Can not be deleted.
            msg.setSeverity(FacesMessage.SEVERITY_WARN);
            FacesContext.getCurrentInstance().addMessage(null, msg);
    ...
    public void nameValidator(FacesContext facesContext, UIComponent uIComponent, Object object) {
        ...
        if (object != null&&object.toString().length()>0) {
            ...
            String naName=object.toString().toUpperCase();
             BindingContainer bindings = getBindings();
            OperationBinding op = bindings.getOperationBinding("naDulicateValidator");
            op.getParamsMap().put("name", naName);
            op.execute();   
            String result=op.getResult().toString();
            if(result.equalsIgnoreCase("N")) {
              throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,resolvEl("#{bundle['MSG.375']}").toString(), null));  //Duplicate Record Exists.
            }
            else{
           for (char c : xx) {
                if (c == '(') {
            ...
            if (openB != closeB || closeFg == true || (langDesc.lastIndexOf("(") > langDesc.lastIndexOf(")")) ||
                ...
                msg2 = "#{bundle['MSG.7']}";   //Brackets not closed properly.
                FacesMessage message2 = new FacesMessage(resolvEl(msg2));
                message2.setSeverity(FacesMessage.SEVERITY_ERROR);
                throw new ValidatorException(message2);
            }
            if (langDesc.contains("()")) {
                msg2 = "#{bundle['MSG.170']}";  //Empty Brackets are not allowed.
                FacesMessage message2 = new FacesMessage(resolvEl(msg2));
                message2.setSeverity(FacesMessage.SEVERITY_ERROR);
                throw new ValidatorException(message2);
            }
            if (langDesc.contains("(.") || langDesc.contains("(-") || langDesc.contains("-)")) {
                msg2 = "#{bundle['MSG.171']}";  //Invalid Natural Account name. Check content inside brackets.
                FacesMessage message2 = new FacesMessage(resolvEl(msg2));
                message2.setSeverity(FacesMessage.SEVERITY_ERROR);
                throw new ValidatorException(message2);
            }
           ...
           String inputStr=(String) object; 
            ...
            String error = resolvEl("#{bundle['MSG.1185']}").toString();  //Alphanumeric character Ampersand(&amp;),Brackets, Dots(.),Forward Slash(/),backward Slash(\),comma(,),colon(:), semicolon(;),Percentage(%) and hyphen(-) not allowed.
...
//        System.out.println("MULTIPLE COA CREATE ALLOWED OR NOT :"+flag);
        ...
        try {//System.out.println("coa act :"+currentRow.getAttribute("AccActv"));
            ...
            if(currentRow.getAttribute("AccActv").toString().equals("Y") && flag.equals("Y")){
                ...
                String msg ="#{bundle['MSG.175']}";  //Only ONE(1) COA can be created for a NA whose 'Multiple Ref in COA' property is unchecked.
                FacesMessage message = new FacesMessage(resolvEl(msg)); 
                message.setSeverity(FacesMessage.SEVERITY_INFO);
                FacesContext fc = FacesContext.getCurrentInstance();
                fc.addMessage(null, message);
                ...
                String msg ="#{bundle['MSG.176']}";  //COA Cannot be created for inactive NA.
                FacesMessage message = new FacesMessage(resolvEl(msg)); 
                message.setSeverity(FacesMessage.SEVERITY_INFO);
                FacesContext fc = FacesContext.getCurrentInstance();
                fc.addMessage(null, message);
                                ...
                                if (NA_ID_OBJ!= null) {
                                    ...
                                    if (result != null) {
                                        if (result.toString().equalsIgnoreCase("N")) {
                                            String msg = (String) resolvEl("#{bundle['MSG.2462']}");  //COA is Active for this NA
                                    ...
                                    if (AccType1 != 0) {
                                        ...
                                        if (result1 != null) {
                                            if (result1.toString().equalsIgnoreCase("N")) {
                                                String msg = (String) resolvEl("#{bundle['MSG.2463']}");  //Entity is Inactive for this NA
                                                throw new ValidatorException(new FacesMessage(FacesMessage.SEVERITY_ERROR,
                                                                                              msg,
                                                                                              null));
                                            }
    ...
    public void partOfBudgetVCE(ValueChangeEvent valueChangeEvent) {
        ...
        if(this.part_of_budgeting_form_bind.isSelected()){
           ...
           {
               String msg = resolvEl("#{bundle['MSG.2464']}");  //This is required in case of part of Budgeting.
              FacesContext.getCurrentInstance().addMessage(this.budget_calc_logic_form_bind.getClientId(),new FacesMessage(FacesMessage.SEVERITY_ERROR,msg,null));
